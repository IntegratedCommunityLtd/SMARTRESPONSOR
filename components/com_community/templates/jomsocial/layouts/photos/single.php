...
if (isset($album->groupid) && $album->groupid > 0) $groupId = $album->groupid;
if (isset($album->eventid) && $album->eventid > 0) $eventId = $album->eventid;

if (!CAlbumsHelper::isFixedAlbum($album)) {
    $canEdit = (COwnerHelper::isCommunityAdmin()) || ($album->creator == $my->id) || (isset($groupId) && $groupId && $my->authorise('community.create', 'groups.photos.' . $groupId));
} else {
    if (isset($groupId)) {
    ...
    $canEdit = ($photo->creator == $my->id) || (COwnerHelper::isCommunityAdmin() || ($isGroupAdmin || $isGroupOwner) || $isEventAdmin);
}
