...
function getErrorContainer( el ) {
    ...
    el = $( el );
    wrapper = el.parent('.joms-select--wrapper, .joms-textarea__wrapper, .joms-location__wrapper');
...
function addValidationTrigger() {
    $('.joms-form__group').find('[required]')
        .add( $('.joms-form__group').find('[data-required]') )
        .off('blur.joms-validation').on( 'blur.joms-validation', function( e, callback ) {
            var el      = $( this ),
            ...
            if ( type === 'text' || type === 'password' || tagName === 'select'  || tagName === 'textarea') {
                if ( typeof callback !== 'function' ) {
                    callback = function() {
                        ...
                        if ( verify ) {
                            verify = $( verify );
                            verify.length && $.trim( verify.val() ) && verify.triggerHandler('blur.joms-validation');
                        }
                    };
                }
...
function validate( $form, callback ) {
    ...
    $fields.each(function() {
        var $el = $( this );
        $el.triggerHandler( 'blur.joms-validation', function( result ) {
