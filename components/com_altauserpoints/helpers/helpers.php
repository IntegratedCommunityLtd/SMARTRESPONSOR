	...
	{		
		...
		switch ( $profilechoice )
		{
			case 'ku' :
				...
				WHERE `link`='index.php?option=com_kunena' AND `type`='component' AND `published`='1'";
				$db->setQuery( $q );
			...
			default :
				...
				$db->setQuery( $q );
				$profileitemid  = '&amp;Itemid=' . $db->loadResult();
				$profilLink = JRoute::_( 'index.php?option=com_altauserpoints&amp;view=account&amp;userid=' . $userinfo->referreid . $profileitemid, $xhtml);
				break;	
		}
