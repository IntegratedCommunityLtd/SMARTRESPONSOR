...
class VmvendorModelDashboard extends JModelItem
{
	...
	public function getMysells() 
	{
		...
		vpl.payment_name ,
		vsl.shipment_name ,
		vu.address_type ,  vu.company , vu.title , vu.last_name , vu.first_name , vu.middle_name , vu.address_1 , vu.address_2 , vu.phone_1 , vu.phone_2 , vu.city ,  vu.zip , 
		...
		LEFT JOIN #__virtuemart_paymentmethods_".VMLANG." vpl ON vpl.virtuemart_paymentmethod_id = vo.virtuemart_paymentmethod_id 
		LEFT JOIN #__virtuemart_shipmentmethods_".VMLANG." vsl ON vsl.virtuemart_shipmentmethod_id = vo.virtuemart_shipmentmethod_id 
		...
		LEFT JOIN #__virtuemart_order_userinfos voui ON voui.virtuemart_order_id = vo.virtuemart_order_id 
			AND voui.address_type='BT' 
		...
		$mysells = $this->_getList($q, $limitstart, $limit);
		return array($mysells, $total, $limit, $limitstart);
	}
	...
	{
		$db = JFactory::getDBO();
		$q ="SELECT vc.`currency_symbol` , vc.`currency_positive_style` , vc.`currency_decimal_place` 
		, vc.`currency_decimal_symbol` , vc.`currency_thousands` 
	...
	public function getMyreviews() 
	{
		...
		$limit 			= $app->getUserStateFromRequest('com_vmvendor.limit', 'limit', $app->getCfg('list_limit'), 'int');
		$limitstart 	= $app->input->get('limitstart', 0, 'INT');		
		$q ="SELECT vrr.virtuemart_rating_review_id , vrr.virtuemart_product_id, vrr.comment 
		, vrr.review_rating , vrr.created_on ,
		vrr.published , 
		u.id , u.".$naming." , 
		vpc.virtuemart_category_id , 
		vp.product_sku ,
	...
	public function getMyTaxes() 
	{
		...
		$my_vendorid = VmvendorModelDashboard::getVendorid();
		$q ="SELECT vc.`virtuemart_calc_id` , vc.`virtuemart_vendor_id`,  vc.`calc_name` , vc.`calc_descr` ,
		 vc.`calc_kind` , vc.`calc_value_mathop` , vc.`calc_value` , vc.`calc_currency` ,  vc.`ordering`  
		 ,  vc.`shared` 
