...
class VmvendorControllerDashboard extends VmvendorController
{
	...
	function updateorderstatus()
	{
		...
		{
			$q = "UPDATE `#__virtuemart_order_items` 
			SET `order_status`='".$neworderstatus."' 
			WHERE `virtuemart_order_item_id`='".$orderitemid."' ";
			$db->setQuery($q);
			if (!$db->execute()) 
				die($db->stderr(true));
			else{
				...
				if ( $neworderstatus=='X')
				{ 
					// we cancel the AUP payment.
					$q = "SELECT `points` FROM `#__vmvendor_userpoints_details` 
					WHERE `keyreference` ='".$orderitemid."|OderItemID' AND `userid`='".$user->id."' ";
					$db->setQuery( $q );
					...
					$q = "SELECT COUNT(id) FROM #__vmvendor_userpoints_details 
					WHERE keyreference ='".$referencekey."' AND userid='".$user->id."' ";
					$db->setQuery($q);
					...
					if(!$yetrefunded)
					{
						...
						$db->setQuery($q);
						$db->execute();
						$q = "UPDATE #__vmvendor_userpoints SET points = points - ".$points2deduct." 
				...
				if ($update_notifies_admin OR $update_notifies_customer OR $neworderstatus=='X')
				{  
					// if status is 'canceled', notify admin even if notifications disabled
					// email admin	
	...
	function publishreview()
	{
		...
		$review_id					= $app->input->post->getInt('review_id');
		if($manage_reviews)
		{
			$q ="UPDATE `#__virtuemart_ratings` SET `published` ='1' 
			WHERE `virtuemart_rating_id`='".$review_id."' ";
			$db->setQuery($q);
			$db->execute();
			$q ="UPDATE `#__virtuemart_rating_reviews` SET `published` ='1' 
			WHERE `virtuemart_rating_review_id`='".$review_id."' ";
			$db->setQuery($q);
			$db->execute();
			
			$q = "SELECT `virtuemart_product_id` FROM  `#__virtuemart_rating_reviews` 
			WHERE `virtuemart_rating_review_id`='".$review_id."' ";
			$db->setQuery($q);
			...
			$q = "SELECT review_rates  
			 ...
			 $q= "UPDATE `#__virtuemart_ratings`  
			 SET rates='".$published_review_total."' , ratingcount	='".$published_review_count."' , 
			 rating='".($published_review_total/$published_review_count)."' 
	...
	function deletereview()
	{
		...
		if($manage_reviews)
		{
			...
			WHERE `virtuemart_rating_vote_id`='".$review_id."' ";
			$db->setQuery($q);
			$db->execute();
	...
	public function deleteshipment()
	{
		...
		$shipment_id	= $app->input->post->getInt('delete_shipmentid');
		$shipment_userid= $app->input->post->getInt('userid');
