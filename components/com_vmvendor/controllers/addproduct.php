	...
	public function addproduct()
	{
		...
		if( $vendor_plan->max_products >0 && $vendor_products_count >= $vendor_plan->max_products )
		{
				$app->enqueueMessage( sprintf( 
					JText::_('COM_VMVENDOR_PLAN_MAXPRODREACHED') , 
					$vendor_plan->max_products , 
					$vendor_plan->title ) , 'warning'
				);
				return false;
		}
		...
		$file1name			= JFile::makeSafe($file1['name']);
			...
			if($first_image)
			{
				...
				if( $first_image['name']!='')
				{
					...
					if ( (substr($first_image['type'],0,5) != 'image' ))
					{
						//image not valid
						$app->redirect(
							'index.php?option=com_vmvendor&view=addproduct&Itemid='.$app->input->get('Itemid'),
							'<i class="vmv-icon-cancel"></i> '.JText::_('COM_VMVENDOR_VMVENADD_IMGEXTNOT'),
							'warning'
							);	
					}							
				}						
		...
		{ // We add the product
			...
			if ($checknotyetadded <1)
			{
				...
				$q = "SELECT `virtuemart_vendor_id` , `user_is_vendor` FROM `#__virtuemart_vmusers` 
				...
				if(!$virtuemart_vendor_id OR $virtuemart_vendor_id=='0')
				{  
				...
				else // way to check if a p has ever been published or not. (announcements and notifications)
				...
				if($enabledimensions)
					$q .= " '".$formlength."','".$formwidth."' ,'".$formheight."','".$formdimensionsunit."', ";		 
				...
				if($enable_vm2tags && $formtags!='')
				{
					...
					if(count($septags) > $tagslimit)
						$app->enqueueMessage(  '<img src="'.$juri.'components/com_vmvendor/assets/img/warning.png" width="16" height="16" alt="" align="absmiddle" /> <b>'.$tagslimit.'</b> '.JText::_('COM_VMVENDOR_VMVENADD_FIRSTTAGSONLY'), 'warning');
					...
					{ 
						...
						if($file)
						{
							...
							if($filename!=''){							
								if	(!$fileisvalid)
								{
									$app->enqueueMessage( '<i class="icon-cancel">
									</i> '.JText::_('COM_VMVENDOR_FILEEXTNOT'),'warning' );
								}
								if($formfilesize > $maxfilesize OR $formfilesize==0)
								{
									$fileisvalid = 0;
									$app->enqueueMessage(  '<i class="icon-cancel">
									</i> '.JText::_('COM_VMVENDOR_MAXFILESIZEX').' '.$formsku ."_".$filename , 'warning' );
								}
							}
							else
								$fileisvalid = 0;
							...
							if($fileisvalid)
							{
								if( JFile::upload($file['tmp_name'] , $target_filepath ) )
								{
									$app->enqueueMessage( 
									'<i class="icon-ok">
									</i> '.JText::_('COM_VMVENDOR_FILEUPLOADRENAME_SUCCESS').' '. $formsku.'_'. $filename, 'message');
								}
								else
								{
									$app->enqueueMessage( '<i class="icon-cancel">
									</i> '.JText::_('COM_VMVENDOR_FILEUPLOAD_ERROR') , 'warning' );
									//$fileisvalid = 0;	
								}
							}
							...
							{	
								$q = "INSERT INTO `#__virtuemart_medias` 
									...
									`file_url` , `file_url_thumb` , `file_is_downloadable` , `file_is_forSale` , `published` , `created_on` , `created_by` )
									VALUES
								...
								{ // product is not free, file is For sale and has to be added as a ST42_download custom plugin entry
										////// mp3
									if( strtolower(substr($filename,-4)=='.mp3') && $resample_commercial_mp3)
									{
										...
										$db->setQuery($q);
										$db->execute();
										$app->enqueueMessage( '<i class="icon-ok"></i> '.JText::_('COM_VMVENDOR_PREVIEWCREATEDSUCCESSFULLY').' '. ($mp3sample_end - $mp3sample_start) .'"', 'message');	
									}
									if($forsalefiles_plugin == 'istraxx')
									{  // Istraxx plugin is used
								...
								{  ////////// Product is free, file is shown on product details
									$q = "INSERT INTO `#__virtuemart_product_medias` 
									( `virtuemart_product_id` , `virtuemart_media_id` , `ordering`)
									VALUES
							...
							elseif($i==1 && $filemandatory)
							{ // files are mandatory and first file field is not valid . EXIT PRODUCT ADDITION!
								$admitted = 0;	
							}
						}
					}
				...
				if($admitted == 0)
				{// we unstorez the product id from the DB
				...
				if($admitted == 1)
				{
					...
					{ 					
						...
						{
							//$app->enqueueMessage($active_languages[$i], 'message'); //en-GB
					...
					if( $formprice < $minimum_price )
					{
						...
						$app->enqueueMessage(  '<img 
						src="'.$juri.'components/com_vmvendor/assets/img/warning.png" 
						width="16" height="16" alt="" 
						align="absmiddle" /> '.JText::_('COM_VMVENDOR_VMVENADD_PRICECHANGED').' '.$minimum_price , 'message');
					}
					...
					if( $formprice>0)
					{
						...
						( `virtuemart_product_id` , `virtuemart_shoppergroup_id` , `product_price` ,
						`override` ,`product_override_price` ,  `product_currency` , `created_on` , `created_by` ) 
						VALUES 
						( '".$virtuemart_product_id."' , '' , '".$formprice."' ,'".$override."','".$product_override_price."', '".$currency_id."' , '".date('Y-m-d H:i:s')."' , '".$user->id."')";
						$db->setQuery($q);
						$db->execute();	
					}
					...
					{ ////////////// images
						...
						if($image['name']!='')
						{
							...
							{
								$app->enqueueMessage(  '<i class="icon-cancel">
								</i> '.JText::_('COM_VMVENDOR_VMVENADD_IMGEXTNOT'),'warning' );
								$imgisvalid = 0;
							}
							...
							if($imgisvalid)
							{
								...
								{
									$app->enqueueMessage( '<i class="icon-cancel">
									</i> '.JText::_('COM_VMVENDOR_VMVENADD_IMAGEUPLOAD_ERROR') ,'warning');
								}
							}
					...
					if($customfields_autoadd)
					{
						...
						{
							$q ="SELECT custom_value FROM #__virtuemart_customs 
							WHERE virtuemart_custom_id='".$customfields_autoadd_id[$i]."' AND published='1' ";
							$db->setQuery($q);
					...
					$db->setQuery($q);
					$virtuemart_custom_id = $db->loadResult();
					if($virtuemart_custom_id && !in_array($virtuemart_custom_id , $customfields_autoadd_id ) )
					{
					...
					$dispatcher->trigger( 'onVMVProductAddition', array( $virtuemart_product_id , $formcat,
																			$formname, $form_s_desc , 
					...
					if($formpublished)
					{
						$app->enqueueMessage( '<i class="vmv-icon-ok"></i> '.JText::_('COM_VMVENDOR_PRODUCTADDED') , 'message');
						$app->enqueueMessage( '<a href="'.JRoute::_('index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id='.$virtuemart_product_id.'&virtuemart_category_id='.$formcat.'&Itemid='.$vmitemid).'"><i class="icon-cube"></i> '.stripslashes(ucfirst($formname) ).'</a>','message');	
					}
					else
