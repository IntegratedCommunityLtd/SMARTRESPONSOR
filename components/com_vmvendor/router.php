...
class VmvendorRouter extends JComponentRouterBase
{
	public function build(&$query)
	{
		...
		if(isset($query['view']))
		{
			...
			if($query['view'] == 'vendorprofile' 
				...
				|| $query['view'] == 'comment'
				|| $query['view'] == 'commentform'
				|| $query['view'] == 'comments' 
				|| $query['view'] == 'withdrawpoints'
			)
			{
				$segments[] = $query['view'];
			}
		...
		{			
			...
			LEFT JOIN `#__virtuemart_vmusers` vvu 
				ON vvu.`virtuemart_vendor_id` = vvl.`virtuemart_vendor_id` 
			WHERE vvu.`virtuemart_user_id` ='".$query['userid']."' ";
			$db->setQuery($sqlQuery);				
			$segments[] = urlencode($query['userid'].'-'.JFilterOutput::stringURLSafe($db->loadResult()));		
			unset($query['userid']);
		}
	...
	public function parse(&$segments)
	{
		...
		if ( $count )
		{
			...
			elseif($segments[0] == 'commentform')
			{
				$vars['view'] = 'commentform';
				return $vars;
			}
			elseif($segments[0] == 'withdrawpoints')
			{
				$vars['view'] = 'withdrawpoints';
				return $vars;
			}
