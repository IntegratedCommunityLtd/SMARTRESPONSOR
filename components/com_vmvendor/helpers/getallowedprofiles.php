...
abstract class VmvendorAllowedProfiles
{
	static function getJSProfileallowed($profiletypes_ids) 
	{
        ...
        if(!$allowed)
		{
			$app->enqueueMessage( JText::_('COM_VMVENDOR_JSPROFILE_NOTALLOWED'). ' <input type="button" class="btn" 
			name="cancel" id="cancelbutton" value="'.JText::_('JCANCEL').'" onclick="history.go(-1)">' , 'message');
		}
	...
	static function getESProfileallowed($profiletypes_ids) 
	{
        ...
        if(!$allowed)
		{
			$app->enqueueMessage( JText::_('COM_VMVENDOR_JSPROFILE_NOTALLOWED'). ' <input type="button" 
			class="btn" name="cancel" id="cancelbutton" value="'.JText::_('JCANCEL').'" 
			onclick="history.go(-1)">' , 'message');	
		}
