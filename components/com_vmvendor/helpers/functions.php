...
abstract class VmvendorFunctions
{
	...
	static function updateRSS( $state , $virtuemart_vendor_id )
	{
		...
		$q = "SELECT vm.file_title , vm.file_url_thumb FROM #__virtuemart_medias vm 
			...
			{
				$app->enqueueMessage( '<i class="vmv-icon-feed"></i> '.JText::_('COM_VMVENDOR_RSSUPDATED').' <a target="_blank" href="'.$juri.'media/vmvendorss/'.$user->id.'.rss" class="hasTooltip" title="'.JText::_('COM_VMVENDOR_RSS_SHAREFEED').'" ></a>', 'message');
			}
	}
	...
	static function emailFlickr( $virtuemart_product_id , $formname , $form_s_desc , $limited_tags , $product_url , $flickr_img)
	{
		...
		$mailerror = '<i class="vmv-icon-cancel"></i> '.JText::_('COM_VMVENDOR_FLICKREMAIL_FAIL');		
		$mailer->setSubject( $flickr_subject );
		$mailer->setBody($flickr_body);
		$mailer->addAttachment( $flickr_img );
		...
		$mailer->addRecipient( array( $config->get( 'mailfrom' ), $flickr_autopost_email )  );
		$sent = $mailer->send();
		if ($sent != 1) 
			echo  $mailerror;
		else
			$app->enqueueMessage('<i class="vmv-icon-flickr"></i> '.JText::_('COM_VMVENDOR_FLICKREMAILED') , 'message');
	...
	static function createVendor( $user_id  , $vendor_currency )
	{
		if (!class_exists( 'VmConfig' ))
			require JPATH_ADMINISTRATOR . '/components/com_virtuemart/helpers/config.php';
		VmConfig::loadConfig();
		...
		VALUES 
		('".$db->escape($user_naming)."' , '".$vendor_currency."' , '' , 'vendor_min_pov=0|vendor_min_poq=1|vendor_freeshipment=0|vendor_address_format=\"\"|vendor_date_format=\"\"|' , '".date('Y-m-d H:i:s')."' , '".$user_id."' , '".date('Y-m-d H:i:s')."' , '".$user_id."' , '0000-00-00 00:00:00' , '0') ";
		$db->setQuery($q);
		$db->execute();		
		...
		FROM #__virtuemart_vmusers WHERE virtuemart_user_id='".$user_id."' ";
		$db->setQuery($q);
		...
		{ // auto add the jomsocial profile applciation
			...
			if($app_added <1)
			{
				...
				('".$user->id."' , 'vmvendor' , '0' , 'content' )"; 	
				 $db->setQuery($q);
				$db->execute();	
					$app->enqueueMessage(JText::_('COM_VMVENDOR_USERPROFILE_APPLICATION_ADDED'), 'message');	
			}	
	...
	static function emailNotifyAddition( $virtuemart_product_id, $formcat , $autopublish )
	{
		...
		$mailerror = '<i class="vmv-icon-cancel"></i> '.JText::_('COM_VMVENDOR_EMAIL_FAIL');		
		...
		$sent = $mailer->send();
		if ($sent !== true) 
			echo  $mailerror.' '.$send->__toString();;
	}
	
	static function getVMItemid()
	{
		$lang = JFactory::getLanguage();
		$app 	= JFactory::getApplication();
		$db 	= JFactory::getDBO();
		$q = "SELECT id FROM #__menu 
		WHERE (link LIKE 'index.php?option=com_virtuemart&view=virtuemart%' 
				OR link LIKE 'index.php?option=com_virtuemart&view=category%' )
		AND type='component'  AND ( language ='".$lang->getTag()."' OR language='*') AND published='1' ";
		$db->setQuery($q);
		$itemid = $db->loadResult();
		if(!$itemid)
			$app->enqueueMessage('VMVendor message: You must set and publish a menu item to the Virtuemart Frontpage or Category page (set to all languages or to this specific language).','warning');
		else
			return $itemid;
	}
}