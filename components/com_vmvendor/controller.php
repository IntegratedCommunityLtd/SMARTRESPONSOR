		...
		if($allow_deletion >0 && $delete_productid    && $user->id >0 && $userid==$user->id   )
		{
			...
			switch($allow_deletion)
			{
				case 1: // unpublish
					...
					$db->setQuery($q);
					$db->execute();
					$message = '<i class="vmv-icon-ok"></i> '.JText::_( 'COM_VMVENDOR_PROFILE_PRODUCTUNPUBLISHED' );
				...
				{ 					
                	for($i = 0 ; $i < count( $active_languages ) ; $i++)
					{
						...
						if( str_replace('_' , '-' , VMLANG) != strtolower( $active_languages[$i]) )
						{
							$q = "DELETE FROM `#__virtuemart_products_".strtolower( str_replace('-' , '_' , $active_languages[$i]) ) ."` 
							 WHERE `virtuemart_product_id`='".$delete_productid."' ";
							$db->setQuery($q);
							$db->execute();	
						}	
			...
			// delete product related activity stream
			if($profileman =='js')
			{ 
