...
$product_vendor_id		= $this->product_data->virtuemart_vendor_id;
$product_sku 			= $this->product_data->product_sku;
$product_weight			= $this->product_data->product_weight;
$product_weightunit		= $this->product_data->product_weight_uom;

$product_length			= $this->product_data->product_length;
$product_width			= $this->product_data->product_width;
$product_height			= $this->product_data->product_height;
$product_dimensionsunit	= $this->product_data->product_lwh_uom;


$product_in_stock		= $this->product_data->product_in_stock;
$published				= $this->product_data->published;
$created_on				= $this->product_data->created_on;
$product_s_desc			= $this->product_data->product_s_desc;
$product_desc			= $this->product_data->product_desc;
$product_name			= $this->product_data->product_name;
$virtuemart_category_id = $this->product_data->virtuemart_category_id;
$product_price 			= $this->product_data->product_price;
$override 				= $this->product_data->override;
$product_override_price = $this->product_data->product_override_price;
...
$enabledimensions 			= $cparams->get('enabledimensions', 1);
$dimensionsunits 			= $cparams->get('dimensionsunits');
$enableprice				= $cparams->get('enableprice', 1);
$enable_priceoverride 		= $cparams->get('enable_priceoverride', 0);
$force_price 				= $cparams->get('force_price');
...
if($enable_priceoverride && !$force_price)
{
	echo 'if (isNaN (it.formpriceoverride.value))
		{
			warning += " * '.JText::_('COM_VMVENDOR_VMVENADD_JS_UNVALIDPRICE').' \n";
			it.formpriceoverride.style.backgroundColor = \'#ff9999\';
		}';	
}
...
if($enabledimensions){
		echo ' if (it.formlength.value=="" || isNaN (it.formlength.value)){
				warning += " * '.JText::_('COM_VMVENDOR_VMVENADD_JS_LENGTHREQUIRED').' \n";
				document.getElementById("formlength").style.backgroundColor = \'#ff9999\';
			} ';	
		echo ' if (it.formwidth.value=="" || isNaN (it.formwidth.value)){
				warning += " * '.JText::_('COM_VMVENDOR_VMVENADD_JS_WIDTHREQUIRED').' \n";
				document.getElementById("formwidth").style.backgroundColor = \'#ff9999\';
			} ';	
		echo ' if (it.formheight.value=="" || isNaN (it.formheight.value)){
				warning += " * '.JText::_('COM_VMVENDOR_VMVENADD_JS_HEIGHTREQUIRED').' \n";
				document.getElementById("formheight").style.backgroundColor = \'#ff9999\';
			} ';	
			echo ' if (it.formdimensionsunit.value=="" ){
				warning += " * '.JText::_('COM_VMVENDOR_VMVENADD_JS_DIMENSIONSUNITREQUIRED').' \n";
				document.getElementById("formdimensionsunit").style.backgroundColor = \'#ff9999\';
...
echo  '<form name="add" enctype="multipart/form-data" onsubmit="return validateForm(this);" method="post" class="vmv-form form-inline" >
...
if($enablemanufield)
{
	...
	<select id="formmanufacturer" name="formmanufacturer" class="form-control" 
	onchange="this.style.backgroundColor = \'\'">
	<option value="0">'.JText::_('COM_VMVENDOR_VMVENADD_FORM_CHOOSEMANUFACTURER').'</option>';
...
if ($enable_sdesc)
{
	...
	echo '<br /><b><span id=myCounter>'. ( 255 - strlen($product_s_desc) ).' </span>
	</b> '.JText::_('COM_VMVENDOR_VMVENADD_FORM_CHARSREMAINING');
	echo '</td>';
	...
	echo  '<td><textarea name="form_s_desc" id="form_s_desc" cols="45" rows="5" 
	onkeyup="this.style.backgroundColor = \'\';return taCount(this,\'myCounter\');" 
	onKeyPress="return taLimit(this)" class="form-control" >';
	echo $product_s_desc;
	echo  '</textarea></td></tr>';
}
...
{
	echo  '<td><textarea name="formdesc" id="formdesc" cols="45" rows="5" 
	onkeyup="this.style.backgroundColor = \'\'" class="form-control">';
	echo $product_desc;
	echo  '</textarea></td>';
}
echo  '</tr>';
...
if($enableweight)
{
					...
					if(count($weightunits)<1)
					{
						...
						{			
							...
							echo '<input type="hidden" id="formweightunit" name="formweightunit" 
							value="'.$product_weightunit.'" />';
						}
						else
						{
					...
					elseif( ( count($weightunits)==1 && !$product_weightunit ) OR ( count($weightunits)==1 && $product_weightunit == $weightunits[0]) )
					{
							...
							echo '<input type="hidden" id="formweightunit" name="formweightunit" 
							value="'.$weightunits[0].'" class="form-control"/>';
					}
...
if($enabledimensions)
{
					echo  '<tr  ><td>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_LENGTH').' <b>*</b><br />
					'.JText::_('COM_VMVENDOR_VMVENADD_FORM_WIDTH').' <b>*</b><br />
					'.JText::_('COM_VMVENDOR_VMVENADD_FORM_HEIGHT').' <b>*</b></td>';
					echo  '<td>';
					echo '<div class="form-group">';
					echo '<input type="text" class="form-control" ';
					if($product_length)
						echo 'value="'.$product_length.'" ';
					echo 'name="formlength" id="formlength" onkeyup="this.style.backgroundColor = \'\';" /><br />';
					echo '<input type="text" class="form-control" ';
					if($product_width)
						echo 'value="'.$product_width.'" ';
					echo 'name="formwidth" id="formwidth" onkeyup="this.style.backgroundColor = \'\';" /><br />';
					echo '<input type="text" class="form-control" ';
					if($product_height)
						echo 'value="'.$product_height.'" ';
					echo 'name="formheight" id="formheight" onkeyup="this.style.backgroundColor = \'\';" />';
					echo '</div>';
					if(count($dimensionsunits)<1)
					{
						if($product_dimensionsunit)
						{			
							echo JText::_('COM_VMVENDOR_VMVENADD_FORM_DIMENSIONS_'.$product_dimensionsunit);
							echo '<input type="hidden" id="formdimensionsunit" name="formdimensionsunit" value="'.$product_dimensionsunit.'" />';
						}
						else
						{
							echo '<br />'.JText::_('COM_VMVENDOR_VMVENADD_FORM_DIMENSIONS_UNITNOTDEFINED');
						}
						
					}
					elseif( ( count($dimensionsunits)==1 && !$product_dimensionsunit ) OR ( count($dimensionsunits)==1 && $product_dimensionsunit == $dimensionsunits[0]) )
					{
							echo JText::_('COM_VMVENDOR_VMVENADD_FORM_DIMENSIONS_'.$dimensionsunits[0]);
							echo '<input type="hidden" id="formdimensionsunit" name="formdimensionsunit" value="'.$dimensionsunits[0].'" class="form-control"/>';
					}
					elseif( count($dimensionsunits)>1  OR !in_array($product_dimensionsunit,$dimensionsunits) )
					{
						echo ' <div class="form-group">';
						echo '<select id="formdimensionsunit" name="formdimensionsunit" onchange="this.style.backgroundColor = \'\';" class="form-control" >';
						echo '<option value="" >'.JText::_('COM_VMVENDOR_VMVENADD_FORM_DIMENSIONS_SELECTUNIT').'</option>';
						foreach($dimensionsunits as $dimensionsunit)
						{
							echo '<option value="'.$dimensionsunit.'" ';
							if($product_dimensionsunit == $dimensionsunit)
								echo ' selected="selected" ';
							echo '>';
							echo JText::_('COM_VMVENDOR_VMVENADD_FORM_DIMENSIONS_'.$dimensionsunit);
							echo '</option>';
						}
						if (!in_array($product_dimensionsunit,$dimensionsunits) && $product_dimensionsunit!='')
						{
							echo '<option value="'.$product_dimensionsunit.'"  selected="selected" >';
							echo JText::_('COM_VMVENDOR_VMVENADD_FORM_DIMENSIONS_'.$product_dimensionsunit);
							echo '</option>';	
						}
...
if($enableprice)
{
	...
	if(!$force_price)
	{
		echo '<input type="text" name="formprice" id="formprice" 
		value="'.number_format($product_price , $currency_decimal_place , '.','').'" 
		onkeyup="this.style.backgroundColor = \'\'" class="form-control"/>';
	}
	else
	{
		echo '<input type="text" name="formprice" id="formprice" 
		value="'.number_format($force_price , $currency_decimal_place , '.','').'" readonly="readonly"/>';
	}
	echo '<label for="formprice">'.$symbol.' ( '.$currency.' )</label>';
	if($currency_mode)
	{
		echo ' <a href="'.JRoute::_('index.php?option=com_vmvendor&view=editprofile').'">
		<i class="vmv-icon-info-sign '.$tipclass.'" 
		title="'.JText::_('COM_VMVENDOR_VMVENADD_FORM_CHOOSECURRENCY').'"></i></a>';		
	}
	echo '<INPUT type="hidden" value="'.$currency.'" name="currency">
...
if($enable_priceoverride && !$force_price )
{
	echo  '<tr ><td>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_PRICEOVERRIDE').'</td>';
	echo  '<td><div class="form-group"><input type="text" name="formpriceoverride" 
	id="formpriceoverride" value="'.number_format($product_override_price , $currency_decimal_place , '.','').'" 
	onkeyup="this.style.backgroundColor = \'\'" class="form-control"/></div>
	<div class="form-group"><label for="formpriceoverride">'.$symbol.' ( '.$currency.' )</label>';
	if($currency_mode)
	{
		echo ' <a href="'.JRoute::_('index.php?option=com_vmvendor&view=editprofile').'">
		<i class="vmv-icon-info-sign '.$tipclass.'" 
...
{////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if($filemandatory)////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		echo  '<b>*</b>';
	echo '<i class="vmv-icon-info-sign '.$tipclass.'" title="('.$allowedexts.') '.JText::_('COM_VMVENDOR_VMVENADD_FORM_MAX').': '.$maxfilesizemega.'"></i>';
	echo  '</td>';
	...
	foreach($this->product_files as $product_file)
	{
		...
		{
			echo '<div title="'.JText::_('COM_VMVENDOR_VMVENADD_FORM_CHECKTOREMOVEFILE').'" class="'.$tipclass.'">';
		...
		{
			echo '<div  >';
			echo '<input type="checkbox"  class="checkbox '.$tipclass.'" title="'.JText::_('COM_VMVENDOR_VMVENADD_FORM_CANTDELETEFIRSTFILE').'" disabled >';
			echo '<i class="vmv-icon-trash"></i>';
			echo '</div>';
		}
...
echo ' <i class="vmv-icon-info-sign '.$tipclass.'" title="(png,gif,jpg) '.JText::_('COM_VMVENDOR_VMVENADD_FORM_MAXSIDE').' '.$maximgside.'px"></i>';
echo '</td>';
echo  '<td>';
$k = 1;
...
foreach($this->product_images as $product_image)
{
	...
	echo '<img src="'.$juri.$product_image->file_url_thumb.'" height="25" class="'.$tipclass.'" title="<img src=\''.$juri.$product_image->file_url_thumb.'\' /><br />'.$product_image->file_title.'"/>';
	...
	if($k>1)
	{
		echo '<div title="'.JText::_('COM_VMVENDOR_VMVENADD_FORM_CHECKTOREMOVEIMAGE').'" class="'.$tipclass.'">';
	...
	{
		echo '<div  class="'.$tipclass.'" title="'.JText::_('COM_VMVENDOR_VMVENADD_FORM_CANTDELETEFIRSTIMG').'">';
...
if($enable_vm2tags)
{
	...
	echo  '<tr >';
	echo '<td>';
	echo JText::_('COM_VMVENDOR_VMVENADD_FORM_TAGS').' <i class="vmv-icon-tags '.$tipclass.'" title="'.JText::_('COM_VMVENDOR_VMVENADD_FORM_TAGSLIMIT').'"></i>';
	echo  '</td>';
					
	echo '<td >';
	if( file_exists(JPATH_BASE.'/components/com_vm2tags/vm2tags.php') )
	{
	...
	{
		echo '<p class="well bg-danger"><i class="vmv-icon-cancel"></i> VM2tags component missing. 
		...
		<a class="btn btn-primary"  target="_blank"		href="https://www.nordmograph.com/extensions/index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id=56&virtuemart_category_id=4&Itemid=58">
...
if($enable_vm2geolocator)
{
	...
	<td>';
	echo JText::_('COM_VMVENDOR_VMVENADD_FORM_LOCATION').' <i class="vmv-icon-location '.$tipclass.'" title="'.JText::_('COM_VMVENDOR_VMVENADD_FORM_LOCATIONDESC').'"></i>';
	echo  '</td>
	<td >';
	if( file_exists(JPATH_BASE.'/plugins/vmcustom/vm2geolocator/vm2geolocator.php') )
	{
		// get the vm2geolocator custom plugin parameters
		$api_url = '//maps.googleapis.com/maps/api/js?&libraries=places&key='.$js_apikey;
		...
		echo '<div class="form-group"><a href="javascript:initgmap();" class="btn btn-sm btn-default" '.$tipclass.'t" title="'.JText::_('COM_VMVENDOR_VMVENADD_FORM_RESET').'"> <i class="vmv-icon-refresh"></i></a></div>';
	}
	else
	{
		...
		href="https://www.nordmograph.com/extensions/index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id=45&virtuemart_category_id=4&Itemid=58">
...
if($enable_embedvideo)
{
	foreach($this->getEmbedvideoFields as $vid_field)
	{
			...
			if($vid_field->custom_tip)
				echo ' <i class="vmv-icon-info-sign '.$tipclass.'" title="'.$vid_field->custom_tip.'" ></i>';
			echo '</td>';
			echo  '<td>';
...
if($enable_corecustomfields)
{
	...
	foreach ($this->core_custom_fields as $core_custom_field)
	{
						...
						if($core_custom_field->custom_tip !='' OR $core_custom_field->custom_desc!='' )
							echo ' <i class="vmv-icon-info-sign '.$tipclass.'" title="'.JText::_($core_custom_field->custom_tip).'"></i>';
						echo  '</td>';
