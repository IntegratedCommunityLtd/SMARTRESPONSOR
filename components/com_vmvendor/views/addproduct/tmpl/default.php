...
$multicat	 	= $cparams->get('multicat', 2);
...
$virtuemart_vendor_id = $this->virtuemart_vendor_id;
	...
	echo 'if (it.formterms.checked==false)
		...
		else
		{
			...
			echo  '<tr ><th>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_PRODUCTINFO').'</th>
			...
			echo  '<tr  >';
				...
				else
				{
					...
					echo '<div class="badge badge-warning '.$tipclass.'" title="'.JText::_('COM_VMVENDOR_VMVENADD_FORM_TOBEMODERATED').'"><i class="vmv-icon-clock"></i> '.JText::_('JUNPUBLISHED').'</div>';
				}
				echo '</td>';
				echo  '</tr>';
	...
	if($multicat=='1' || $multicat=='2') //multi or classic
    {
        ...
        function traverse_tree_down( $class , $category_id , $level , $forbidcatids , $onlycatids , $virtuemart_vendor_id)
        {
                ...
                $level++;
                $q = "SELECT * FROM `#__virtuemart_categories_".VMLANG."` AS vmcl, 
				`#__virtuemart_category_categories` AS vmcc,   `#__virtuemart_categories` AS vmc
                    ...
                    AND vmc.`published`='1' 
                    AND (vmc.`virtuemart_vendor_id`='1' OR vmc.`virtuemart_vendor_id` ='".$virtuemart_vendor_id."' 
					OR vmc.`shared`='1' ) ";
    ...
    elseif($multicat=='0') // multi steps ebay style cateogry selector    
    {
        function traverse_tree_down( $class , $category_id , $level , $forbidcatids , $onlycatids , $virtuemart_vendor_id , $toplevel)
        {
                ...
                if(count($cats)>0)
                {
                    ...
                    {
                        echo '<option value="'.$cat->virtuemart_category_id.'" ';
						//if(count($cats)=='1') echo ' selected="selected" ';
                        echo '> ';
...
///////////////////////////////// end Category select field
        ...
        elseif($cat_suggest==3) // inserted published
	{
	 ...
	 {
		 $modaltype='s';
		 $iframe_url = 'index.php?option=com_vmvendor&view=catsuggest&tmpl=component';
		 if($modaltype=='b')
		 {
			 //JHtml::_('bootstrap.framework');//breaks page js
			$name = "CatsuggestModal";
			echo  '<div style="float:right;">
			<a href="#modal-' . $name.'" data-toggle="modal" 
			class="btn  btn-primary btn-mini " 
			title="'.$suggest_message.'">
			<i class="vmv-icon-plus" ></i></a></div>';
			$params = array();
		...
		echo '<div style="float:right;"><a class="btn btn-primary btn-mini modal " 
		title="'.$suggest_message.'" 
		...
		<i class="vmv-icon-plus" ></i></a> </div>';
		  ...
		  elseif($modaltype=='s')
		 {
			...
			$addcat_script = "function addcat(){
			swal({  
				  html:
					'<iframe src=\"".$iframe_url."\" id=\"catsuggest_iframe\" >' +
					'</iframe>',
					width: 600,
					padding:0,
			...
			$doc->addScriptDeclaration($addcat_script);
			 echo  '<div style="float:right;">
			<a href="#" 
			class="btn  btn-primary btn-mini "  onclick="addcat()";
			title="'.$suggest_message.'">
			<i class="vmv-icon-plus" ></i></a></div>';
	...
	{
				echo  '<tr  >';
				echo  '<td>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_LENGTH').' <b>*</b><br />
				'.JText::_('COM_VMVENDOR_VMVENADD_FORM_WIDTH').' <b>*</b><br />
				'.JText::_('COM_VMVENDOR_VMVENADD_FORM_HEIGHT').' <b>*</b></td>';
				echo  '<td>';
				echo '<div class="form-group">';
				echo '<input type="text" name="formlength" id="formlength" class="form-control" onkeyup="this.style.backgroundColor = \'\';" /><br />
				<input type="text" name="formwidth" id="formwidth" class="form-control" onkeyup="this.style.backgroundColor = \'\';" /><br />
				<input type="text" name="formheight" id="formheight" class="form-control" onkeyup="this.style.backgroundColor = \'\';" />';
				echo '</div>';
				
				if(count($dimensionsunits)<1)
				{
					echo '<br />'.JText::_('COM_VMVENDOR_VMVENADD_FORM_DIMENSIONS_UNITNOTDEFINED');
					echo '<input type="hidden" id="formdimensionsunit" class="form-control" name="formdimensionsunit" value="NA" />';
				}
				elseif(count($dimensionsunits)==1)
				{
					echo JText::_('COM_VMVENDOR_VMVENADD_FORM_DIMENSIONS_'.$dimensionsunits[0]);
					echo '<input type="hidden" id="formdimensionsunit" class="form-control" name="formdimensionsunit" value="'.$dimensionsunits[0].'" />';	
				}
				elseif(count($dimensionsunits)>1)
				{
					echo ' <div class="form-group">';
					echo '<select id="formdimensionsunit" name="formdimensionsunit" class="form-control" onchange="this.style.backgroundColor = \'\';" >';
	...
	if($enablestock)
	{
		...
		<td><div class="form-group"><input type="text" size="6" name="formstock" 
		id="formstock" class="form-control form-inline" 
		onkeyup="this.style.backgroundColor=\'\';this.value=this.value.replace(/\D/,\'\')" 
		/></div></td></tr>';
	...
	if($enableprice)
	{
		echo  '<tr  ><td>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_PRICE').' <b>*</b>';
		
		echo '</td>';
		echo  '<td>';
		if(!$force_price)
		{
			echo '<div class="form-group"><input type="text" name="formprice" id="formprice" 
			class="form-control" onkeyup="this.style.backgroundColor = \'\'" /> <label 
			for="formprice">'.$symbol.' ( '.$currency.' )</label><INPUT type="hidden" 
			value="'.$currency_id.'" name="formcurrency">';
			if($currency_mode)
			{
				echo ' <a href="'.JRoute::_('index.php?option=com_vmvendor&view=editprofile').'"><i class="vmv-icon-info-sign '.$tipclass.'" 
		...
		{
			echo '<div class="form-group"><input type="text" name="formprice" id="formprice" 
			value="'.$force_price.'" readonly />
			<label 
			for="formprice">'.$symbol.' ( '.$currency.' )</label><INPUT type="hidden" 
			value="'.$currency_id.'" name="formcurrency"></div>';	
		}
	...
	if($enable_priceoverride && !$force_price )
	{
				...
				echo  '<td>
				<div class="form-group"><input type="text" name="formpriceoverride" id="formpriceoverride" 
				class="form-control" onkeyup="this.style.backgroundColor = \'\'" /></div>
	...
	if($enablefiles)
	{
		...
		echo ' <i class="vmv-icon-info-sign '.$tipclass.'" title="('.$allowedexts.') '.JText::_('COM_VMVENDOR_VMVENADD_FORM_MAX').': '.$maxfilesizemega.'"></i> <div class="badge">'.JText::_('COM_VMVENDOR_PLAN_MAX').': '.$max_filefields.'</div>';
		echo  '</td>';
	...
	if($enable_vm2tags)
    {
		...
		<i class="vmv-icon-info-sign '.$tipclass.'" title="'.JText::_('COM_VMVENDOR_VMVENADD_FORM_TAGSLIMIT').': '.$tagslimit.'::'.JText::_('COM_VMVENDOR_VMVENADD_FORM_TAGSCOMASEPARATED').'"></i></td>';	
		echo '<td >';
	...
	{// get the vm2geolocator custom plugin parameters
		...
		<i class="vmv-icon-location '.$tipclass.'" title="'.JText::_('COM_VMVENDOR_VMVENADD_FORM_LOCATIONDESC').'"></i>';
		echo  '</td>';
		...
		$be_maptype	= get_between($vm2geo_params, 'default_maptype="', '"|');
		if(!$be_maptype) $be_maptype = 'ROADMAP';
		$doc->addScript( "//maps.googleapis.com/maps/api/js?libraries=places&key=".$js_apikey);
		...
		$mapscript ="function add_Event(obj_, evType_, fn_){ 
			...
			echo '<div class="form-group"><a href="javascript:initgmap();" class="btn btn-sm btn-default '.$tipclass.'"  title="'.JText::_('COM_VMVENDOR_VMVENADD_FORM_RESET').'"> <i class="vmv-icon-refresh"></i></a></div>';
			echo '</div>';
		}
		else
		{
 ...
 href="https://www.nordmograph.com/extensions/index.php?option=com_virtuemart&view=productdetails&virtuemart_product_id=45&virtuemart_category_id=4&Itemid=58">
		...
		foreach ($this->core_custom_fields as $core_custom_field)
		{
			...
			switch($core_custom_field->field_type)
			{
				case "S":  //string
					...
					{
						echo '<input name="corecustomfield_'.$i.'" id="corecustomfield_'.$i.'" 
						type="text" value="'.$core_custom_field->custom_value.'" size="50" class="form-control" />';
					}
					break;
				case "I": // integer
					echo '<input name="corecustomfield_'.$i.'" id="corecustomfield_'.$i.'"  
				...
				break;
				case "B": // bolean
					echo '<div class="radio-inline"><input name="corecustomfield_'.$i.'" 
					id="corecustomfield_'.$i.'_0" type="radio"   value="0"  ';
					if($core_custom_field->custom_value =='0' )
						echo ' checked="checked" ';
					echo  ' /> <label for="corecustomfield_'.$i.'_0">'.JText::_('JNo').'</label></div>';
					echo  '<div class="radio-inline"><input name="corecustomfield_'.$i.'" 
					id="corecustomfield_'.$i.'_1"   type="radio"    value="1"';
				...
				break;
				case "T": // time
					echo '<input class="form-control" name="corecustomfield_'.$i.'" 
					id="corecustomfield_'.$i.'" type="text" value="'.$core_custom_field->custom_value.'" size="50"  />';
				break;
				case "M": // image
				...
				case "V": // cart variant
					if(!$core_custom_field->is_list)
					{
						echo '<input name="corecustomfield_'.$i.'" type="text" 
						value="'.$core_custom_field->custom_value.'" size="50" class="form-control" />';
					}
					else
					{
						...
						if(count($exploded_cartvar)>1)
						{
							...
							<option value="">'.JText::_('COM_VMVENDOR_SELECT_OPTION').'</option>';
				...
				case "X": // editor
					jimport( 'joomla.html.editor' );
					$editor = JFactory::getEditor();
					$editor_customfield_html = $editor->display("corecustomfield_".$i , 
															$core_custom_field->custom_value , 
															"100%;", 
															'200', 
															'5', 
															'30', 
				...
				case "Y": // textarea
					...
					echo $core_custom_field->custom_value.'</textarea>';			
				break;
			}
			echo '</td>';
			echo  '</tr>';
	...
	if ($user->id != 0) {
        ...
        $app->enqueueMessage(JText::_('COM_VMVENDOR_VMVENADD_ONLYLOGGEDIN'), 'message');
    }
    echo '</td>';
	echo  '</tr>';
	echo  '</table>';
