...
$shipmentid = $app->input->getInt('shipmentid');
...
echo '<div class="well"><i class="vmv-icon-info-sign">
</i> '.JText::_('COM_VMVENDOR_EDITSHIPMENT_FORM_TAXEDITION_NOTICE').'</div>';

$virtuemart_shipmentmethod_id   = $app->input->get('virtuemart_shipmentmethod_id','','int');
$jpluginid                    	= $app->input->get('jpluginid','','int');

	?>
    <form  method="post" name="adminForm" id="adminForm"  class="vmv-form form-validate form-horizontal"  action="<?php echo JRoute::_('index.php?option=com_vmvendor&view=editshipment&Itemid='.$app->input->get('Itemid') ) ?>" >
