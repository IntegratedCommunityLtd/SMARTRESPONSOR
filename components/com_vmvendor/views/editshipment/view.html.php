...
class VmvendorViewEditshipment extends JViewLegacy
{
	function display($tpl = null) 
	{
		...
		if($shipment_mode!=1)
		{
			$app->enqueueMessage( JText::_('COM_VMVENDOR_EDITSHIPMENT_SHIPMENT_MODE_DISABLED'). ' <input type="button" 
			class="btn" name="cancel" id="cancelbutton" value="'.JText::_('JCANCEL').'" onclick="history.go(-1)">' ,'warning');
			return false;	
		}
		if(JFactory::getUser()->id<1)
		{
			$app->enqueueMessage( JText::_('COM_VMVENDOR_VMVENADD_ONLYLOGGEDIN'). ' <input type="button" 
			class="btn" name="cancel" value="'.JText::_('JCANCEL').'" onclick="history.go(-1)">' ,'warning');
			return false;	
		}
