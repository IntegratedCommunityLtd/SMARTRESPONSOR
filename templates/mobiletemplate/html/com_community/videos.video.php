			...
			<div class="span8">
				<div class="cMain">
					...
					<?php if($wallCount > 0 || !empty($wallForm) ) {?>
						...
						</div>						<!-- JComments ON -->	<?php   global $mainframe;		$comments = $mainframe->getCfg('absolute_path'). '/components/com_jcomments/jcomments.php';			if (file_exists($comments)) {				require_once($comments);				echo JComments::showComments($video->id, 'com_community_v', $title);			}	?><!-- JComments END -->
					<?php } ?>
				</div><!--cMain-->
			</div>
