...
<?php //if ($showToolbar) : ?>
	<div class="row sm">
		<div class="col-xs-2">
			<button type="button" class="btn btn-link navbar-toggle joms-trigger__menu--main">
		...
		<div class="col-xs-2 text-right">
			<button type="button" class="btn btn-link pull-right joms-trigger__menu--user">
	...
	<div class="row full">
	<div class="col-sm-1" style="margin: 0;padding:0px;"></div>
	<div class="col-sm-1" style="width:175px;padding-right: 5px;padding-left: 0px;margin-left: -15px;float: left;margin: 0;"></div>
	<div class="col-sm-8" style="margin: 0;padding:0px;">
							<?php
							// Возврат модуля Поиска
								$modules =JModuleHelper::getModules('navbar-search');
									foreach ($modules as $module){
										echo JModuleHelper::renderModule($module);
									}
							?>	
			<ul class="nav navbar-nav navbar-right hidden-xs pane">
							<?php
							// Возврат модуля Авторизации
								$modules =JModuleHelper::getModules('navbar-sign');
									foreach ($modules as $module){
										echo JModuleHelper::renderModule($module);
									}
							?>				
							<?php
							// Возврат модуля Профиля меню
								$modules =JModuleHelper::getModules('navbar-avatar');
									foreach ($modules as $module){
										echo JModuleHelper::renderModule($module);
									}
							?>
							...
							<?php
							// Возврат модуля Главное меню
								$modules =JModuleHelper::getModules('navbar-main-menu');
									foreach ($modules as $module){
										echo JModuleHelper::renderModule($module);
									}
							?>	
				...
				?>
				<li class="hidden-xs <?php echo $active === $menu->item->id ? 'active ' : '' ?><?php echo (isset($menu->item->css)) ? $menu->item->css : '' ?>">
					<a href="<?php echo CRoute::_($menu->item->link) ?>"<?php echo $attr; ?>><?php echo JText::_( $menu->item->name ); ?></a>
					<?php if ($hasDropdown) { ?>
					<ul class="hidden-xs joms-dropdown joms-arrow--top-left">
				...
				*/	
				...
				<li class="hidden-xs">
					...
					<ul class="hidden-xs joms-popover joms-arrow--top joms-popover--toolbar-friendrequest">
						<li class="joms-js--loading" style="display:block">
							<img src="<?php echo JURI::root(true); ?>/components/com_community/assets/ajax-loader.gif" alt="loader">
						</li>
					</ul>
				...
				<li class="hidden-xs">
					...
					<ul class="hidden-xs joms-popover joms-popover--toolbar-search">
						<li class="joms-js--noremove joms-js--field">
							<input type="text" class="joms-input" placeholder="<?php echo JText::_('COM_COMMUNITY_SEARCH'); ?>"
								oninput="joms.view.toolbar.search(this, event);">
						</li>
						<li class="hidden-xs joms-js--noremove joms-js--loading">
							<img src="<?php echo JURI::root(true); ?>/components/com_community/assets/ajax-loader.gif" alt="loader">
						</li>
						<li class="hidden-xs joms-js--noremove joms-js--viewall">
							<div>
								<a href="javascript:" class="joms-button--neutral joms-button--full"
									data-lang="<?php echo JText::_('COM_COMMUNITY_VIEW_ALL_N_RESULTS'); ?>"></a>
								<form method="post" action="<?php echo CRoute::_('index.php?option=com_community&view=search'); ?>">
				...
				*/
				...
				<li class="hidden-xs">
					<a href="#" data-toggle="dropdown"><span class="glyphicon glyphicon-chevron-down"></span></a>
							<?php
							// Возврат модуля Дополнительное меню
								$modules =JModuleHelper::getModules('navbar-other-menu');
									foreach ($modules as $module){
										echo JModuleHelper::renderModule($module);
									}
							?>		
...
<div class="joms-menu">
					<?php
					// Возврат модуля Дополнительное меню
						$modules =JModuleHelper::getModules('left-panel-menu');
							foreach ($modules as $module){
								echo JModuleHelper::renderModule($module);
							}
					?>
...
<div class="joms-menu--user">
					<?php
					// Возврат модуля Дополнительное меню
						$modules =JModuleHelper::getModules('right-panel-menu');
							foreach ($modules as $module){
								echo JModuleHelper::renderModule($module);
							}
					?>
