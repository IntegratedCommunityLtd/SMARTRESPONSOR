...
$enableAlbumLocation = (!empty($location)) && ($config->get('photosmapdefault') == 1);
...
<div class="joms-page">
    <h3 class="joms-text--title"><?php echo CStringHelper::escape($album->get('name')); ?></h3>
    ...
    <button class="joms-button--add-on-page joms-button--primary joms-button--small"
            onclick="joms.api.photoUpload('<?php echo $album->id; ?>', '<?php echo ($groupId) ? $groupId : '' ?>');"><?php echo JText::_('COM_COMMUNITY_PHOTOS_UPLOAD_PHOTOS_TO_ALBUM'); ?></button>
    <?php } ?>
        ...
        <ul class="joms-list--photos">
            <?php
                ...
                for ($j = 0; $j < count($photos); $j++) {
                    ...
                    $photoUrl = CRoute::_('index.php?option=com_community&view=photos&task=photo&albumid=' . $row->albumid . '&photoid=' . $row->id . ( $groupId ? '&groupid=' . $groupId : '' ));
                    if ( $isPhotoModal ) {
...
<?php } ?>
        ...
        <?php if (!empty($otherAlbums)) { ?>
            ...
            <h3 class="joms-text--title">
                <?php echo JText::_( $groupId > 0 ? 'COM_COMMUNITY_PHOTOS_GROUP_OTHER_ALBUMS' : 'COM_COMMUNITY_PHOTOS_OTHER_ALBUMS'); ?>
            </h3>
            ...
            <ul class="joms-gallery">
                <?php foreach ($otherAlbums as $others) { ?>
                    ...
                    <li class="joms-gallery__item">
                        <div class="joms-gallery__thumbnail">
                            <a href="<?php echo $albumURL; ?>">
                                <img src="<?php echo $others->getCoverThumbURI(); ?>" alt="">
                            </a>
                        </div>
