...
<div class="joms-focus__cover">
    ...
    <div class="joms-focus__cover-image joms-js--cover-image">
        <img src="<?php echo $group->getCover(); ?>" style="width:100%;top:<?php echo $group->coverPostion; ?>" alt="<?php echo $group->name; ?>" >
    </div>
    ...
    <ul class="joms-dropdown">
        ...
        if ( ($isMember && !$isBanned && !$waitingApproval) || $isSuperAdmin) { ?>
            ...
            if ($isAdmin || $isSuperAdmin || $isMine) { ?>
                ...
                <li class="joms-js--menu-reposition"<?php echo $group->defaultCover ? ' style="display:none"' : '' ?>>
            ...
            if ($my->authorise('community.create', 'groups.photos.' . $group->id)) { ?>
                <li>
                    <a href="javascript:" data-propagate="1" onclick="joms.api.photoUpload('', '<?php echo $group->id; ?>');"><?php echo JText::_('COM_COMMUNITY_PHOTOS_UPLOAD_PHOTOS'); ?></a>
                </li>
            <?php } ?>
            ...
            if ($my->authorise('community.create', 'groups.videos.' . $group->id)) { ?>
                <li>
                    <a href="javascript:" data-propagate="1" onclick="joms.api.videoAdd('<?php echo $group->id; ?>');"><?php echo JText::_('COM_COMMUNITY_VIDEOS_ADD'); ?></a>
                </li>
            <?php } ?>
    ...
    <div class="joms-module__wrapper">
        <div class="joms-tab__bar">
            <?php if($allowDiscussion) { ?>
                <a href="#joms-group--discussion" class="active"><?php echo JText::_('COM_COMMUNITY_DISCUSSIONS'); ?></a>
                <?php
                $isFirstTab = 0;
            }?>

            <?php if($allowAnnouncement) { ?>
                <a href="#joms-group--announcement" class="<?php if($isFirstTab) echo "active";?>"><?php echo JText::_('COM_COMMUNITY_ANNOUNCEMENTS'); ?></a>
            <?php } ?>
        </div>
        <?php if($allowDiscussion) { ?>
                                        ...
                                        <div class="joms-comment__header">
                                            <div class="joms-avatar--comment">
                                                <a href="<?php echo CUrlHelper::userLink($row->creator->id); ?>">
                                                    <img src="<?php echo $row->creator->getThumbAvatar(); ?>" alt="<?php echo $row->creator->getDisplayName(); ?>"/>
                                                </a>
                                            </div>
...
<?php if ($group->approvals == '0' || $isMine || ($isMember && !$isBanned) || $isSuperAdmin) { ?>
    ...
    <?php if ($showVideos) { ?>
        ...
        <?php if ($videos) { ?>
            <div class="joms-module__wrapper">
                <div class="joms-tab__bar">
                    <a href="#joms-group--videos" class="active"><?php echo JText::_('COM_COMMUNITY_VIDEOS'); ?></a>
                </div>
