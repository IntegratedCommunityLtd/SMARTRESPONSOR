...
$statusupdate_alert="function sweetStatusUpdate(it)
					{
						swal(
							{   title: '".JText::_( 'COM_VMVENDOR_DASHBOARD_CONFIRMSTATUSUPDATE1' )."', 
...
if(!$use_as_catalog)
{
	echo JHtml::_('bootstrap.addTab', 'dashboardTab', 'mysells', '<i class="vmv-icon-cart x-large"></i><p> '.JText::_('COM_VMVENDOR_DASHBOARD_MYSALES').'</p>' , 'class="active"' );
	echo '<table class="table table-responsive category table-striped table-condensed table-hover table-bordered">';
...
<button id="fullscreen-btn-table" type="button" class="btn btn-link" return false"><span class="glyphicon glyphicon-fullscreen"></span><span class="visible-xs">Âî âåñü ýêðàí</span></button>

	id="fullscreen-btn-table" åñòü ñòèëè â ôàéëå bootstrap.css â êîðíå ñàéòà*/
	echo '</tr>';
	echo '<tr>';
	...
	foreach($this->mysales as $sale)
	{
		...
		if($allow_orderstatuschange){
			...
			if($sale->order_status =='C'   ){
				...
				if(!$hasfile)
				{
					...
					id="statusform'.$sale->virtuemart_order_item_id.'" >';
					echo '<select name="neworderstatus" 
					onchange="sweetStatusUpdate( document.getElementById(\'statusform'.$sale->virtuemart_order_item_id.'\')) ;" >';
		...
		if($sale->virtuemart_user_id ){
			...
			if($profileman!='0')	
				echo '<div><a href="'.$profile_url.'"  title="'.JText::_( 'COM_VMVENDOR_DASHBOARD_VISITPROFILE' ).'" class="btn btn-mini btn-default hasTooltip">
		...
		if($profileman=='js' && $customercontactform == 2 && $sale->virtuemart_user_id !='0' && $sale->virtuemart_user_id )
		{
			...
			//echo  '<a href="javascript:void(0)" onclick="javascript: joms.messaging.loadComposeWindow('.$sale->virtuemart_user_id.');" title="'.JText::_( 'COM_VMVENDOR_DASHBOARD_PMCUSTOMER' ).'" class="btn btn-mini btn-default jomNametips">';
		...
		{
			echo  '<a href="javascript:void(0)" data-es-conversations-compose data-es-conversations-id="'.$sale->virtuemart_user_id.'" title="'.JText::_( 'COM_VMVENDOR_DASHBOARD_PMCUSTOMER' ).'" class="btn btn-mini btn-default" data-es-provide="tooltip">';
			echo  '<i class="vmv-icon-mail"></i></a>';
		}
		else
		{
			$name = "mailcustomer";
			$html = '<a href="#modal-' . $name.'" data-toggle="modal" class="btn btn-mini" title="'.JText::_('COM_VMVENDOR_VENDORLINK_ASK'). '"><i class="vmv-icon-mail"></i></a>';
			$params = array();
		...
		if($sale_phone_1)
			echo '<div title="'.$sale_phone_1.'" class=" btn btn-mini btn-default hasTooltip"><i class="vmv-icon-phone"></i></div>';
		if($sale_phone_2)
			echo '<div title="'.$sale_phone_2.'" class=" btn btn-mini btn-default hasTooltip"><i class="vmv-icon-phone"></i></div>';	
...
if ( !$use_as_catalog && $show_pointstab) {
	echo JHtml::_('bootstrap.addTab', 'dashboardTab', 'mypoints', '<i class="vmv-icon-star x-large"></i><p> '.JText::_('COM_VMVENDOR_DASHBOARD_MYPOINTSTITLE').'</p>'  );
...
if(!$use_as_catalog){
	...
	echo JHtml::_('bootstrap.addTab', 'dashboardTab', 'mystats', '<i class="vmv-icon-chart x-large"></i><p>'.JText::_('COM_VMVENDOR_DASHBOARD_STATISTICS').'</p>'  );
		...
		echo '<div class="form-group">'.JText::_( 'COM_VMVENDOR_DASHBOARD_STATSSTARTDATE' ).' '.JHTML::_('calendar', $start_date, 'start_date', 'start_date',  '%Y-%m-%d', array('class'=>'inputbox form-control nomargin', 'size'=>'5',  'maxlength'=>'10')).'</div>'; 
		...
		echo '<div class="form-group"><select id="time_unit" name="time_unit" class="form-control nomargin">';
...
{	
	...
	if(count($this->myreviews) >0)
	{
		...
		foreach($this->myreviews as $review)
		{
			...
			if(!$review->published){
				...
				$tr .='<input type="image" src="'.$juri.'components/com_vmvendor/assets/img/good.png" name="image" title="'.JText::_( 'COM_VMVENDOR_DASHBOARD_PUBLISH').'" alt="Publish" class="hasTooltip" width="16" height="16" onclick="sweetPublishReview(document.getElementById(\'publish_review'.$review->virtuemart_rating_review_id.'\') )" >';
			...
			$tr .='<a title="'.JText::_( 'COM_VMVENDOR_DASHBOARD_DELETE').'" class="hasTooltip btn btn-mini btn-danger" 
	...
	echo JHtml::_('bootstrap.addTab', 'dashboardTab', 'productreviews', '<i class="vmv-icon-comment x-large"></i><p>'.$panel_title.'</p>' );
	echo '<div>';
	if(count($this->myreviews)>0){
...
{
	echo JHtml::_('bootstrap.addTab', 'dashboardTab', 'mytaxes', '<i class="vmv-icon-barcode x-large"></i><p> '.JText::_('COM_VMVENDOR_DASHBOARD_TAXES').'</p>'  );
	if($tax_mode==2)
	{
			echo '<div class="right_addbutton" >';
			echo '<a href="'.JRoute::_('index.php?option=com_vmvendor&view=edittax&Itemid='.JRequest::getInt('Itemid')).'" class="btn btn-mini btn-default">';
...
if($shipment_mode)
{
	echo JHtml::_('bootstrap.addTab', 'dashboardTab', 'shipments', '<i class="vmv-icon-truck x-large"></i><p> '. JText::_( 'COM_VMVENDOR_DASHBOARD_SHIPMENT_MANAGER').'</p>'  );	
	echo '<div class="right_addbutton">';
	echo '<a href="'.JRoute::_('index.php?option=com_vmvendor&view=editshipment&jpluginid='.$this->shipmentextensionid).'" class="btn btn-mini"><i class="vmv-icon-plus"></i> '.JText::_('COM_VMVENDOR_DASHBOARD_SHIPMENT_ADD').'</a>';
	echo '</div>';
	echo '<div style="clear:both;"></div>';
	...
	//		echo '<a class="hasTooltip">'..'</a>';
			...
			echo '<form name="deleteshipment'.$shipment->virtuemart_shipmentmethod_id.'" id="deleteshipment'.$shipment->virtuemart_shipmentmethod_id.'" method="post" >';
			echo '<div class="btn-group">';

			echo '<a class="btn btn-mini btn-primary hasTooltip" title="'.JText::_('COM_VMVENDOR_DASHBOARD_SHIPMENT_EDIT').'" href="'.$editshipment_url.'">
			<i class="vmv-icon-edit"></i></a>';
			...
			echo '<input type="hidden" name="controller" value="dashboard">
				...
				<input type="hidden" name="userid" value="'.$user->id.'">
				<a class="btn btn-mini btn-danger hasTooltip" title="'.JText::_('COM_VMVENDOR_DASHBOARD_SHIPMENT_TRASH').'" 
...
jQuery(document).ready(function($){
    $("#dashboardTabTabs").removeClass('nav nav-tabs');
	$("#dashboardTabTabs").addClass('nav nav-pills nav-justified');
});
</script>