...
if($enablestock)
{
	echo ' if (it.formstock.value=="" || isNaN (it.formstock.value))
			{
				warning += " * <h4>'.JText::_('COM_VMVENDOR_VMVENADD_JS_STOCKREQUIRED').'</h4> \n";
...
echo '<form name="add" enctype="multipart/form-data" onsubmit="return validateForm(this);" method="post" class="editpublic" ><dl class="dl-horizontal">
<div class="row"><div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"><h4>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_PRODUCTINFO').'</h4></div>
<div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"><b>*</b> '.JText::_('COM_VMVENDOR_VMVENADD_MANDATORYFIELDS').'</div></div>';

//multilang by icon
$lang = JFactory::getLanguage();
	
if (!$current_lang = JRequest::getVar('product_lang', false)) {
	$current_lang = strtolower(str_replace('-', '_', $lang->getTag()));
}
	
$languages = JLanguage::getKnownLanguages();
$img = explode('_', $current_lang);

echo '<div class="row"><div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"><h4>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_LANG').'</h4></div><div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"><div class="form-group input-group"><span class="input-group-addon"><img alt="'.$img[0].'" src="/media/mod_languages/images/'.$img[0].'.gif" /></span><select onchange="window.location.href = window.location.href.replace(\'&product_lang='.$current_lang.'\', \'\') + \'&product_lang=\' + this.options[this.selectedIndex].value;" name="product_lang" class="form-control chosen nomargin">';
foreach ($languages as $lang => $row) {
	$langf = strtolower(str_replace('-', '_', $lang));
	$selected = '';
	if ($current_lang == $langf) {
		$selected = ' selected="selected"';
	}
			
	echo '<option'.$selected.' value="'.$langf.'">'.$row['name'].'</option>';	
}
...
echo '<div class="col-xs-12 col-sm-12 com-md-6 col-lg-4">';
...
{				
	...
	echo '> <label for="unpublished"><h4>'.JText::_('JUNPUBLISHED').'</h4></label></div>
							<div class="badge badge-success"><input type="radio" name="formpublished" id="published" value="1" ';
	if($published) echo ' checked="checked" ';
	echo '> <label for="published"><h4>'.JText::_('JPUBLISHED').'</h4></label></div>';
}
else{
	...
	if($published)
		echo '<div class="badge badge-success"><i class="vmv-icon-ok"></i> <h4>'.JText::_('JPUBLISHED').'</h4></div>';
	else
		echo '<div class="badge badge-warning"><i class="vmv-icon-clock"></i> <h4>'.JText::_('JUNPUBLISHED').'</h4></div>';
}
echo '</div></div>';
echo '<div class="row"><div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"><h4>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_NAME').' <b>*</b></h4></div>
<div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"><input type="text" name="formname" id="formname" size="50" onkeyup="this.style.backgroundColor = \'\'" value="'.$product_name.'" class="form-control" />';
echo '</div></div>';
if($show_sku)
{
	echo '<div class="row"><div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"><h4>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_SKU').'</h4></div>
	<div class="col-xs-12 col-sm-12 com-md-6 col-lg-4">'.$product_sku.'</div></div>';
}
...
if($multicat)
	echo '<h4>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_CATS').'</h4>';
else
	echo '<h4>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_CAT').'</h4>';
echo '</div>';

echo '<div class="col-xs-12 col-sm-12 com-md-6 col-lg-4">';// Category select field
...
$traverse = traverse_tree_down('' , 0 , 0 , $forbidcatids, $onlycatids , $virtuemart_category_id , $virtuemart_vendor_id, $multicat );
	echo '</select>';
	echo '</div></div>';// end Category select field

	echo '<div class="row"><div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"><h4>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_SDESC').' <b>*</b></h4>
	<br /><B><SPAN id=myCounter>'. ( 255 - strlen($product_s_desc) ).' </SPAN></B>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_CHARSREMAINING').'';
	echo '</div>';// COM_VMVENDOR_VMVENADD_FORM_SDESC
	...
	echo '<div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"><textarea name="form_s_desc" id="form_s_desc" cols="45" rows="5" onkeyup="this.style.backgroundColor = \'\';return taCount(this,\'myCounter\');" onKeyPress="return taLimit(this)" class="form-control" >';
	echo $product_s_desc;
	echo '</textarea></div></div>';

echo '<div class="row"><div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"><h4>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_DESC').' <b>*</b></h4></div>';
if($wysiwyg_prod)
{
	...
	echo '<div class="col-xs-12 col-sm-12 com-md-6 col-lg-4">'.$editorhtml.'</div></div>';
}else
{
	echo '<div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"><textarea name="formdesc" id="formdesc" cols="45" rows="5" onkeyup="this.style.backgroundColor = \'\'" class="form-control">';
	echo $product_desc;
	echo '</textarea></div></div>';
}
	echo '<div class="row"><div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"><h4>'.JText::_('COM_VMVENDOR_VMVENADD_MANUFACTURER').'</h4></div>';
	echo '<div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"><select id="formmanufacturer" name="formmanufacturer" class="form-control" onchange="this.style.backgroundColor = \'\'"><option value="0">'.JText::_('COM_VMVENDOR_VMVENADD_FORM_CHOOSEMANUFACTURER').'</option>';
...
if($enableweight)
{
					echo '<div class="row"><div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"><h4>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_WEIGHT').' <b>*</b></h4></div>';
					echo '<div class="col-xs-12 col-sm-12 com-md-6 col-lg-4">';
...
if($enablestock)
{
	echo '<div class="row"><div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"><h4>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_STOCK').' <b>*</b></h4></div>';
	echo '<div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"><input type="text" size="6" name="formstock" id="formstock" value="'.$product_in_stock.'" onkeyup="this.style.backgroundColor = \'\';this.value=this.value.replace(/\D/,\'\')" class="form-control" /></div></div>';
}

if($enableprice)
{
	echo '<div class="row"><div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"><h4>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_PRICE').' <b>*</b></h4></div>';
	echo '<div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"><div class="form-group"><input type="text" name="formprice" id="formprice" value="'.number_format($product_price , $currency_decimal_place , '.','').'" onkeyup="this.style.backgroundColor = \'\'" class="form-control"/></div>
...
{////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if($filemandatory)////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		echo '<b>*</b>';
	echo '<i class="vmv-icon-info-sign hasTooltip" title="('.$allowedexts.') '.JText::_('COM_VMVENDOR_VMVENADD_FORM_MAX').': '.$maxfilesizemega.'"></i></h4>';
	...
	{
		echo '<div id="fileinput'.$m.'" class="input-group fileclonedInput">';
		echo ' <input type="file" name="file'.$m.'" id="file'.$m.'" onchange="this.style.backgroundColor = \'\'" class="form-control nomargin" />';
...
echo ' <i class="vmv-icon-info-sign hasTooltip" title="(png,gif,jpg) '.JText::_('COM_VMVENDOR_VMVENADD_FORM_MAXSIDE').' '.$maximgside.'px"></i></h4>';
$k = 1;

foreach($this->product_images as $product_image)
{
	echo '<img src="'.$juri.$product_image->file_url_thumb.'" class="img-thumbnail">';
	echo '<div id="imginput'.($k ).'_edit" class="input-group imgclonedInput " >';
//	echo '<span class="input-group-addon" data-toggle="popover" data-content="'.$product_image->file_title.'"><img src="'.$juri.$product_image->file_url_thumb.'"></span>';
	echo '<input type="file" name="image'. $k .'" id="image'. $k .'" onchange="this.style.backgroundColor = \'\'" class="form-control nomargin";/>';
	
	echo '<span class="input-group-addon">';
	if($k>1)
	{
		echo '<input type="checkbox" name="delimg'. $k .'" id="delimg'. $k .'" data-toggle="tooltip" class="btn" data-content="'.JText::_('COM_VMVENDOR_VMVENADD_FORM_CHECKTOREMOVEIMAGE').'">';
		//echo '<i class="vmv-icon-trash"></i>';
	}
	else
	{
		echo '<input type="checkbox" disabled data-toggle="tooltip" class="btn " data-content="'.JText::_('COM_VMVENDOR_VMVENADD_FORM_CANTDELETEFIRSTIMG').'">';
		//echo '<i class="vmv-icon-trash"></i>';
	}
	echo '</span>';	
...
{
	echo '<div id="imginput'.($k ).'" class="input-group imgclonedInput" >';
	echo ' <input type="file" name="image'.($k ).'" id="image'.($k ).'" onchange="this.style.backgroundColor = \'\'" class="form-control nomargin"/> ';
if($max_imagefields>1 && $k < $max_imagefields)
{
		echo '<span class="input-group-btn">';
...
{
	echo '<div class="row">';
///	echo '<div class="row"><div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"><h4>'.JText::_('COM_VMVENDOR_VMVENADD_CUSTOMFIELDS').'</h4></div>';
}
if($enable_vm2tags)
{
	echo '<div class="col-xs-12 col-sm-12 com-md-6 col-lg-4">';
	echo '<h4>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_TAGS').' <i class="vmv-icon-tags hasTooltip" title="'.JText::_('COM_VMVENDOR_VMVENADD_FORM_TAGSLIMIT').'"></i></h4>';
	...
	{
		echo '<p class="well bg-danger"><i class="vmv-icon-cancel"></i>plugin and component missing. 
		Disable the option in <a target="_blank" href="">settings</a> or 
...
{
	echo '<div class="row"><div class="col-xs-12 col-sm-12 com-md-6 col-lg-4">';
	echo '<h4>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_LOCATION').' <i class="vmv-icon-location hasTooltip" title="'.JText::_('COM_VMVENDOR_VMVENADD_FORM_LOCATIONDESC').'"></i></h4>';
	echo '</div><div class="col-xs-12 col-sm-12 com-md-6 col-lg-4">';
...
//		echo '<tr class="geolocator" style="background-color:#f7f7f7;">';
		echo '<div class="row"><div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"></div><div class="col-xs-12 col-sm-12 com-md-6 col-lg-4">';
	...
	else
	{
		...
		Disable the option in <a target="_blank" href="administrator/index.php?option=com_config&view=component&component=com_vmvendor">settings</a>
		<a class="btn btn-primary" target="_blank"
		href="">Download</a></p>';
	}	
	echo '</div></div>';	
...
if($enable_embedvideo)
{
	foreach($this->getEmbedvideoFields as $vid_field)
	{
			...
			if( file_exists(JPATH_BASE.'/plugins/vmcustom/embedvideo/embedvideo.php') )
			{
				...
				echo '<i class="vmv-icon-cancel"></i> Embedvideo plugin is not installed.';
			echo '</div></div>';
	}		
}
...
//{
	...
	foreach ($this->core_custom_fields as $core_custom_field)
	{
						...
						echo '<div class="row"><div class="col-xs-12 col-sm-12 com-md-6 col-lg-4">';
						//echo 'Under dev: ';
						echo '<h4>'.JText::_($core_custom_field->custom_title).'';
						
						if($core_custom_field->custom_tip !='' OR $core_custom_field->custom_desc!='' )
							echo ' <i class="vmv-icon-info-sign hasTooltip"  data-toggle="tooltip" data-title="'.JText::_($core_custom_field->custom_tip).'" data-content="'.JText::_($core_custom_field->custom_desc).'"></i></h4>';
						echo '</div>';
						echo '<div class="col-xs-12 col-sm-12 com-md-6 col-lg-4">';
			...
			if($core_custom_field->is_list)
				{
					...
					echo '<select name="corecustomfield_'.$i.'" id="corecustomfield_'.$i.'" class="form-control chosen">';
...
/*
	echo '<div class="row"><div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"></div>';
	echo '<div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"><div id="checkboxtd" class="checkbox inline">
	<input type="checkbox" name="formterms" id="formterms" onchange="document.getElementById(\'checkboxtd\').style.backgroundColor = \'\';"/> <label for="formterms"><h4>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_IAGREE').'</h4> '; 
	if($termsurl !=NULL)
	echo '<a href="'.$termsurl.'" target="_blank" ><h4>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_TERMS').'</h4></a>';
	else
...
if( ( $profileman=='js' || $profileman=='es') && $this->autopublish && $created_on !='0000-00-00 00:00:00')
{
	echo '<div class="row"><div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"></div>';
	echo '<div class="col-xs-12 col-sm-12 com-md-6 col-lg-4">';	
	echo '<div class="checkbox inline" id="announcebox">';	
	echo '<input type="checkbox" name="announceupdate" id="announceupdate" /> <label for="announceupdate" >'.JText::_('COM_VMVENDOR_VMVENADD_ANNOUNCEUPDATE').'</label></div>';
	echo '</div></div>';	
}
...
if($flickr_autopost && $created_on =='0000-00-00 00:00:00')
	{
	echo '<div class="row"><div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"></div>';
	echo '<div class="col-xs-12 col-sm-12 com-md-6 col-lg-4"><i class="vmv-icon-flickr" ></i>';
	echo '<div id="checkboxtd" class="checkbox"><input type="checkbox" name="flickrcheckbox" id="flickrcheckbox" checked/> '.JText::_('COM_VMVENDOR_VMVENADD_FORM_PROMOTEONFLICKR');
 echo ' </div></div></div>';
	}
...
{
	echo'<div class="row">';
	echo'<div class="col-xs-12 col-sm-12 com-md-12 col-lg-12">';
	echo '<button type="submit" name="update" id="button"  class="ntn btn-warning btn-lg" style="width:100%;" value="'.JText::_('COM_VMVENDOR_VMVENADD_BTTN_UPDATE').'" class="btn btn-primary">'.JText::_('COM_VMVENDOR_VMVENADD_BTTN_UPDATE').'</button>';
	echo'</div>';
	echo'</div>';
