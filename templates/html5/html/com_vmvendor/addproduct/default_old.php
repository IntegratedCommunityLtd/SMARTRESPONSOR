...
$virtuemart_vendor_id = $this->virtuemart_vendor_id;
	...
	?>
	<?php echo '<p><strong><b>*</b>'.JText::_('COM_VMVENDOR_VMVENADD_MANDATORYFIELDS').'</strong></p>'; ?>
	<form class="form-horizontal" id="form-center">
	
		<div class="form-group">
			<label class="col-sm-10 col-sm-push-1" for="input-form-1"><?php echo '<strong>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_PRODUCTINFO').' '.JText::_('COM_VMVENDOR_VMVENADD_FORM_NAME').'</strong>'; ?>
				
			</label>
			<div class="col-sm-10 col-sm-push-1"><input type="text" class="form-control" id="input-form-1">
			</div>
		...
		<div class="form-group">
			...
			<label><strong><?php echo JText::_('COM_VMVENDOR_VMVENADD_FORM_PUBLICATION'); ?></strong></label>
			</div>
			<?php
			if($this->autopublish)
				{
			...
			<label><strong>
			<input type="radio" name="optradio" value="0"><?php echo ' '.JText::_('JUNPUBLISHED'); ?>
			</strong></label>
			</div>
			<div class="btn btn-primary">
			<label>
			<input type="radio" name="optradio" value="1" checked="checked"><?php echo ' '.JText::_('JPUBLISHED'); ?>
			</label>
			</div>
			</div>
			...
			<?php 
				...
				{ ?>
				<input type="hidden" name="formpublished" value="0" />
				<div class="col-sm-10 col-sm-push-1">
				<div class="btn btn-primary" title="<?php echo JText::_('COM_VMVENDOR_VMVENADD_FORM_TOBEMODERATED'); ?>"><i class="vmv-icon-clock"></i> <?php echo JText::_('JUNPUBLISHED'); ?> </div>
	...
	?>
	<?php
				echo '<div class="col-sm-10 col-sm-push-1"><label class="control-label"><strong>';
			if($multicat)
            {
	...
	echo '<div class="col-sm-10 col-sm-push-1">';
	if($multicat)
	{
	...
	if($show_sku){
		echo '<div class="col-sm-1 col-sm-push-1">'.JText::_('COM_VMVENDOR_VMVENADD_FORM_SKU').': </div>';
		echo '<div class="text-left col-sm-10 col-sm-push-1">'.$sku.'</div>';
	}
	echo '</div>';
	echo '<div class="row"><label for="formmanufacturer" class="col-sm-10 col-sm-push-1"><strong>'.JText::_('COM_VMVENDOR_VMVENADD_MANUFACTURER').'</strong></label></div>';
    echo '<div class="row"><div class="col-sm-10 col-sm-push-1">';
	echo '<select id="formmanufacturer" name="formmanufacturer" class="form-control vm-chzn-select"><option value="0">'.JText::_('COM_VMVENDOR_VMVENADD_FORM_CHOOSEMANUFACTURER').'</option>';
		...
		echo '<div class="form-group"><label class="col-sm-10 col-sm-push-1"><strong>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_DESC').' <b>*</b></strong></label>';
		...
		if($wysiwyg_prod){
				...
				$editorhtml = $editor->display("formdesc", "", "100%;", '200', '', '', false);
				// ; required after %
				//$editorhtml = JEditor::display( 'editor', '' , 'description', '100%;', '150', '5', '30' );
				echo '<div class="col-sm-10 col-sm-push-1">'.$editorhtml.'</div>';
	}
	else{
				echo '<div class="col-sm-10 col-sm-push-1"><textarea name="formdesc" id="formdesc" class="form-control" required="required" cols="45" rows="7" onkeyup="this.style.backgroundColor = \'\'" style="width:100%;">';
	...
	echo '</div>';
		...
		//Акций
		if($enablestock){
				echo '<div class="form-group"><label class="col-sm-10 col-sm-push-1"><strong>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_STOCK').' <b>*</b></strong></label>';
				echo '<div class="col-sm-10 col-sm-push-1"><input type="text" size="6" name="formstock" id="formstock" class="form-control form-inline" onkeyup="this.style.backgroundColor = \'\';this.value=this.value.replace(/\D/,\'\')" /></div></div>';
	}
			
	//Акция	
	if($enableprice){
				echo '<div class="form-group"><label class="col-sm-10 col-sm-push-1"><strong>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_PRICE').' <b>*</b></strong></label>';
				echo '<div class="col-sm-10 col-sm-push-1"><input type="text" name="formprice" id="formprice" class="form-control" onkeyup="this.style.backgroundColor = \'\'" /></div>
				
				<div class="col-sm-10 col-sm-push-1"><label for="formprice" class="control-label">'.$symbol.' ( '.$currency.' )</label><INPUT type="hidden" class="form-control" value="'.$currency.'" name="currency">
				</div></div>';
	}

echo '<div class="form-group"><label class="col-sm-10 col-sm-push-1"><strong>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_FILES').'</strong></label></div>';


	if($enablefiles)
	{
		echo '<div class="form-group"><div class="col-sm-10 col-sm-push-1">';
      ...
      jQuery('#fileDel').attr('disabled',true);
		...
		echo '<div class="col-sm-8 col-sm-push-1 col-xs-8"><div id="fileinput1" style="margin-bottom:4px" class="fileclonedInput">';
		echo ' <input type="file" name="file1" id="file1" class="form-control" onchange="this.style.backgroundColor = \'\'" />';
		echo '</div>';
		echo '</div>';
		if($max_filefields>1){
			echo '<div class="col-sm-1 col-sm-push-1"><div>
	...
	if($imagemandatory)
  {
	...
	$img_ajax = "jQuery(document).ready(function() {
		...
		echo '<div class="col-sm-8 col-sm-push-1"><div id="imginput1" style="margin-bottom:4px;" class="imgclonedInput">';
		echo ' <input type="file" name="image1" id="image1" class="form-control " onchange="this.style.backgroundColor = \'\'" /> ';
		echo '</div></div>';

if($max_imagefields>1){
			echo '<div class="col-sm-1 col-sm-push-1"><div> <i class="vmv-icon-picture"></i> 
	...
	if($enable_vm2tags OR $enable_vm2geolocator OR $enable_embedvideo OR $enable_corecustomfields){
		...
		if($enable_vm2tags)
  {
		echo '<dt><label for="formtags" ><strong><i class="vmv-icon-tags "></i> '.JText::_('COM_VMVENDOR_VMVENADD_FORM_TAGS').'</strong></label> 
		<i class="vmv-icon-info-sign hasTooltip" title="'.JText::_('COM_VMVENDOR_VMVENADD_FORM_TAGSLIMIT').': '.$tagslimit.'::'.JText::_('COM_VMVENDOR_VMVENADD_FORM_TAGSCOMASEPARATED').'"></i></dt>';	
		echo '<dd>';
	...
	{// get the vm2geolocator custom plugin parameters
		...
		echo '<div class="form-group"><div class="col-sm-10 col-sm-push-1"><strong>';
		echo JText::_('COM_VMVENDOR_VMVENADD_FORM_LOCATION').' 
		<i class="vmv-icon-location hasTooltip" title="'.JText::_('COM_VMVENDOR_VMVENADD_FORM_LOCATIONDESC').'"></i></strong>';
		echo '</div></div>';
		echo '<div class="form-group"><div class="col-sm-10 col-sm-push-1">';
		...
		{
			echo '<div class="form-group"><div class="col-sm-10 col-sm-push-1">';
			echo '<label for="embedvideo_'.$vid_field->virtuemart_custom_id.'" ><strong><i class="vmv-icon-youtube-play" ></i> '.$vid_field->custom_title.' </strong></label>';
			...
			if($enable_corecustomfields)
	{
		...
		{
			$i++;
			echo '<div class="form-group"><label for="corecustomfield_'.$i.'" class="col-sm-10 col-sm-push-1"><strong>';
			...
			switch($core_custom_field->field_type){
				...
				if($core_custom_field->is_list)
				{
					...
					echo '<dt>';				
					echo '<dt>';
					echo '<div class="form-group"><div class="col-sm-10 col-sm-push-1"><select class="form-control vm-chzn-select" name="corecustomfield_'.$i.'" id="corecustomfield_'.$i.'">';
					...
					{
						echo '<div class="form-group"><div class="col-sm-10 col-sm-push-1"><input name="corecustomfield_'.$i.'" id="corecustomfield_'.$i.'" type="text" value="'.$core_custom_field->custom_value.'" size="50" class="form-control" ';
							echo '/></div></div>';
					}
				break;
				case "I": // integer
						echo '<dt>';				
						echo '<dt>';	
						echo '<div class="form-group"><div class="col-sm-10 col-sm-push-1"><input name="corecustomfield_'.$i.'" id="corecustomfield_'.$i.'" type="text" value="'.$core_custom_field->custom_value.'" size="50" class="form-control" /></div></div>';
				break;
				case "B": // bolean
							...
							echo ' /><label for="corecustomfield_'.$i.'_0"><strong>'.JText::_('JNo').'</strong></label></div>';
							
							echo '<dd><div class="radio-inline"><input name="corecustomfield_'.$i.'" id="corecustomfield_'.$i.'_1"  type="radio"  value="1"';
							if($core_custom_field->custom_value =='1' )
								echo ' checked="checked" ';
							echo ' /> <label for="corecustomfield_'.$i.'_1"><strong>'.JText::_('JYes').'</strong></label></div></dd>';
				break;
				case "D": // date
				...
				case "T": // time
							echo '<dt>';				
							echo '<dt>';	
							echo '<div class="form-group"><div class="col-sm-10 col-sm-push-1"><input class="form-control" name="corecustomfield_'.$i.'" id="corecustomfield_'.$i.'" type="text" value="'.$core_custom_field->custom_value.'" size="50" /></div></div>';
				break;
				case "M": // image
				...
				case "V": // cart variant
					if(!$core_custom_field->is_list)
						...
						if(count($exploded_cartvar)>1){
							echo '<dt>';				
							echo '<dt>';	
							echo '<dd><select name="corecustomfield_'.$i.'" id="corecustomfield_'.$i.'" class="form-control vm-chzn-select" >';
				...
				echo '<dt>';				
				echo '<dt>';				
				echo '<dd><textarea name="corecustomfield_'.$i.'" id="corecustomfield_'.$i.'" >';
	...
	if($flickr_autopost)
	{
		echo '<dt style="text-align:right"><i class="vmv-icon-flickr" ></i>';
	echo '</dt><div class="col-sm-10 col-sm-push-1"><div id="checkboxtd" class="checkbox"><input type="checkbox" name="flickrcheckbox" id="flickrcheckbox" checked/> '.JText::_('COM_VMVENDOR_VMVENADD_FORM_PROMOTEONFLICKR'); //
  echo ' </div></div>';
		
	}
	
	echo '<dt>';
	echo '</dt><div class="col-sm-10 col-sm-push-1"><div id="checkboxtd" class="checkbox"><input type="checkbox" name="formterms" id="formterms" onchange="document.getElementById(\'checkboxtd\').style.backgroundColor = \'\';"/> <label for="formterms" ><strong>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_IAGREE').'</strong></label> '; //
	if ($termsurl != NULL) {
