...
$virtuemart_vendor_id = $this->virtuemart_vendor_id;
	...
	echo 'if (it.formterms.checked==false)
		...
		else
		{
			...
			echo '<dt>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_PRODUCTINFO').'</dt><dd><b>*</b>'.JText::_('COM_VMVENDOR_VMVENADD_MANDATORYFIELDS').'</dd>';
			echo '<dt><label for="formname">'.JText::_('COM_VMVENDOR_VMVENADD_FORM_NAME').' <b>*</b></label></dt>';
			echo '<dd>';
...
///////////////////////////////// end Category select field
	...
	if($enable_sdesc)
	{
				echo '<dt><label for="form_s_desc">'.JText::_('COM_VMVENDOR_VMVENADD_FORM_SDESC').'<b>*</b><br /><B><span id=myCounter>255</span></B> '.JText::_('COM_VMVENDOR_VMVENADD_FORM_CHARSREMAINING').'</label></dt>';
				echo '<dd><div class="form-group"><textarea name="form_s_desc" id="form_s_desc" class="form-control" cols="45" rows="5" onkeyup="this.style.backgroundColor = \'\';return taCount(this,\'myCounter\');"  onKeyPress="return taLimit(this)"  ></textarea>';
				echo '</div></dd>';	
	...
	echo '<dt><label for="formdesc" >'.JText::_('COM_VMVENDOR_VMVENADD_FORM_DESC').' <b>*</b></label></dt>';
	if($wysiwyg_prod){
				...
				echo '<dd><textarea name="formdesc" id="formdesc" class="form-control" required="required" cols="45" rows="5" onkeyup="this.style.backgroundColor = \'\'" style="width:100%;height:70px;">';
			//	echo $get_description;
				echo '</textarea></dd>';
	}
	if($enableweight){
				echo '<dt>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_WEIGHT').' <b>*</b></dt>';
				echo '<dd><div class="form-group">';
	...
	if($enablestock){
				...
				echo '<dd><input type="text" size="6" name="formstock" id="formstock" class="form-control form-inline" onkeyup="this.style.backgroundColor = \'\';this.value=this.value.replace(/\D/,\'\')" /></dd>';
	}
	...
	if($enablefiles)
	{
      ...
      jQuery('#fileDel').attr('disabled',true);
		...
		echo '</div>';
		if($max_filefields>1){
			echo '<div style="float:right" class="col-xs-2 col-sm-3 col-md-3 col-lg-4">
		...
		echo '<div id="fileinput1" style="margin-bottom:4px;" class="col-xs-10 col-sm-9 col-md-9 col-lg-8 fileclonedInput">';
	...
	if($imagemandatory)
  {
	...
	$img_ajax = "jQuery(document).ready(function() {
		...
		if($max_imagefields>1){
			echo '<div style="float:right;" class="col-xs-2 col-sm-3 col-md-3 col-lg-4"> <i class="vmv-icon-picture"></i> 
		...
		echo '<div id="imginput1" style="margin-bottom:4px;" class="col-xs-10 col-sm-9 col-md-9 col-lg-8 imgclonedInput">';
	...
	if($enable_vm2tags)
  {
		echo '<dt><label for="formtags" ><i class="vmv-icon-tags "></i> '.JText::_('COM_VMVENDOR_VMVENADD_FORM_TAGS').'</label> 
	...
	if($enable_corecustomfields)
	{
		...
		{
			$i++;
			echo '<dt><label for="corecustomfield_'.$i.'">';
			...
			switch($core_custom_field->field_type){
				...
				if($core_custom_field->is_list)
				{
					...
					//echo '<dt>';				
					//echo '<dt>';
					echo '<dd><select class="form-control vm-chzn-select" name="corecustomfield_'.$i.'" id="corecustomfield_'.$i.'">';
					...
					{
						echo '<dd><input name="corecustomfield_'.$i.'" id="corecustomfield_'.$i.'" type="text" value="'.$core_custom_field->custom_value.'" size="50" class="form-control" ';
							echo '/></dd>';
					}
				break;
				case "I": // integer
						echo '<dt>';				
						echo '<dt>';	
						echo '<dd><input name="corecustomfield_'.$i.'" id="corecustomfield_'.$i.'" type="text" value="'.$core_custom_field->custom_value.'" size="50" class="form-control" /></dd>';
				break;
				case "B": // bolean
							...
							echo ' /><dt><label for="corecustomfield_'.$i.'_0">'.JText::_('JNo').'</label></dt></div>';
							...
							if($core_custom_field->custom_value =='1' )
								echo ' checked="checked" ';
							echo ' /> <label for="corecustomfield_'.$i.'_1">'.JText::_('JYes').'</label></div></dd>';
				break;
				case "D": // date
				...
				case "T": // time
							echo '<dt>';				
							echo '<dt>';	
							echo '<dd><input class="form-control" name="corecustomfield_'.$i.'" id="corecustomfield_'.$i.'" type="text" value="'.$core_custom_field->custom_value.'" size="50" /></dd>';
				break;
				case "M": // image
	...
	echo '</dt><dd><div id="checkboxtd" class="checkbox"><input type="checkbox" name="flickrcheckbox" id="flickrcheckbox" checked/> '.JText::_('COM_VMVENDOR_VMVENADD_FORM_PROMOTEONFLICKR'); //
  echo ' </div></dd>';
		
	}
	
	echo '<dt>';
	echo '</dt><dd><div id="checkboxtd" class="checkbox"><input type="checkbox" name="formterms" id="formterms" onchange="document.getElementById(\'checkboxtd\').style.backgroundColor = \'\';"/> <label for="formterms" >'.JText::_('COM_VMVENDOR_VMVENADD_FORM_IAGREE').'</label> '; //
	if ($termsurl != NULL) {
