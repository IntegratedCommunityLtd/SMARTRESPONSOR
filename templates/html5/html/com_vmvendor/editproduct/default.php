...
<dt>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_PRODUCTINFO').'</dt>
<dd><b>*</b> '.JText::_('COM_VMVENDOR_VMVENADD_MANDATORYFIELDS').'</dd>';
...
echo '<dt>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_PUBLICATION').'</dt>';
echo '<dd>';
...
echo '</dd>';
echo '<dt>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_NAME').' <b>*</b></dt>
<dd><input type="text" name="formname" id="formname" size="50" onkeyup="this.style.backgroundColor = \'\'" value="'.$product_name.'" class="form-control" />';
echo '</dd>';
if($show_sku)
{
	echo '<dt>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_SKU').'</dt>
	<dd>'.$product_sku.'</dd>';
}
...
$traverse = traverse_tree_down('' , 0 , 0 , $forbidcatids, $onlycatids , $virtuemart_category_id , $virtuemart_vendor_id, $multicat );
	echo '</select>';
	echo '</dd>';// end Category select field

	echo '<dt>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_SDESC').' <b>*</b>
	<br /><B><SPAN id=myCounter>'. ( 255 - strlen($product_s_desc) ).' </SPAN></B> '.JText::_('COM_VMVENDOR_VMVENADD_FORM_CHARSREMAINING');
	echo '</dt>';// COM_VMVENDOR_VMVENADD_FORM_SDESC
	...
	echo '<dd><textarea name="form_s_desc" id="form_s_desc" cols="45" rows="5" onkeyup="this.style.backgroundColor = \'\';return taCount(this,\'myCounter\');" onKeyPress="return taLimit(this)" class="form-control" >';
	echo $product_s_desc;
	echo '</textarea></dd>';

echo '<dt>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_DESC').' <b>*</b></dt>';
if($wysiwyg_prod)
{
	...
	echo '<dd>'.$editorhtml.'</dd>';
}else
{
	echo '<dd><textarea name="formdesc" id="formdesc" cols="45" rows="5" onkeyup="this.style.backgroundColor = \'\'" class="form-control">';
	echo $product_desc;
	echo '</textarea></dd>';
}
	echo '<dt>'.JText::_('COM_VMVENDOR_VMVENADD_MANUFACTURER').'</dt>'; // COM_VMVENDOR_VMVENADD_MANUFACTURER
	echo '<dd><select id="formmanufacturer" name="formmanufacturer" class="form-control" onchange="this.style.backgroundColor = \'\'"><option value="0">'.JText::_('COM_VMVENDOR_VMVENADD_FORM_CHOOSEMANUFACTURER').'</option>';
...
if($enableweight)
{
					echo '<dt>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_WEIGHT').' <b>*</b></dt>';
					echo '<dd>';
...
if($enablestock)
{
	echo '<dt>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_STOCK').' <b>*</b></dt>';
	echo '<dd><input type="text" size="6" name="formstock" id="formstock" value="'.$product_in_stock.'" onkeyup="this.style.backgroundColor = \'\';this.value=this.value.replace(/\D/,\'\')" class="form-control" /></dd>';
}

if($enableprice)
{
	echo '<dt>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_PRICE').' <b>*</b></dt>';
	echo '<dd><div class="form-group"><input type="text" name="formprice" id="formprice" value="'.number_format($product_price , $currency_decimal_place , '.','').'" onkeyup="this.style.backgroundColor = \'\'" class="form-control"/></div>
	...
	echo '<dt>'.JText::_('COM_VMVENDOR_VMVENADD_FORM_FILES').'</dt>';
	echo '<dd>';	
if($enablefiles)
{////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
{
	echo '<div id="imginput'.($k ).'_edit" class="imgclonedInput form-group input-group" >';
	...
	echo ' <input type="file" name="image'. $k .'" id="image'. $k .'" onchange="this.style.backgroundColor = \'\'" style="" class="form-control"/>';
	echo '<span class="input-group-addon">';
	if($k>1)
	{
		echo '<input type="checkbox" name="delimg'. $k .'" id="delimg'. $k .'" data-toggle="popover" data-content="'.JText::_('COM_VMVENDOR_VMVENADD_FORM_CHECKTOREMOVEIMAGE').'">';
		//echo '<i class="vmv-icon-trash"></i>';
	}
	else
	{
		echo '<input type="checkbox" disabled data-toggle="popover" data-content="'.JText::_('COM_VMVENDOR_VMVENADD_FORM_CANTDELETEFIRSTIMG').'">';
		//echo '<i class="vmv-icon-trash"></i>';
	}
...
{
	echo '<dt>'.JText::_('COM_VMVENDOR_VMVENADD_CUSTOMFIELDS').'</dt>';
}
if($enable_vm2tags)
{
	echo '<dd>';
...
//{
	...
	foreach ($this->core_custom_fields as $core_custom_field)
	{
						...
						if($core_custom_field->custom_tip !='' OR $core_custom_field->custom_desc!='' )
							echo ' <i class="vmv-icon-info-sign hasTooltip"  data-toggle="popover" data-title="'.JText::_($core_custom_field->custom_tip).'" data-content="'.JText::_($core_custom_field->custom_desc).'"></i>';
						echo '</dt>';
						echo '<dd>';
...
//}
	echo '<dt></dt>';
	echo '<dd><div id="checkboxtd" class="checkbox inline">
...
if($flickr_autopost && $created_on =='0000-00-00 00:00:00')
	{
	echo '<dt></dt>';
	echo '<dd><i class="vmv-icon-flickr" ></i>';
