...
<div class="joms-focus">
    ...
    <div id="profile_cover" class="joms-focus__cover">
        ...
        <div class="carousel-inner0 text-center joms-focus__cover-image joms-js--cover-image">
            <img src="<?php echo $profile->cover; ?>" style="width:100%;top:<?php echo $profile->coverPostion; ?>" alt="<?php echo $user->getDisplayName(); ?>">
        </div>

        <div class="joms-focus__cover-image--mobile" style="background:url(<?php echo $profile->cover; ?>) no-repeat center center;">
        </div>
		
                <a id="profile_avatar"
                    <?php if ( $avatar ) { ?>
                    href="<?php echo CRoute::_('index.php?option=com_community&view=photos&task=photo&albumid=' . $avatar->albumid . '&photoid=' . $avatar->id); ?>"
                    onclick="joms.api.photoOpen('<?php echo $avatar->albumid; ?>', '<?php echo $avatar->id; ?>'); return false;"
                    <?php } ?>>
                    <img class="img-thumbnail" src="<?php echo $profile->largeAvatar . '?_=' . time(); ?>" alt="<?php echo $this->escape( $user->getDisplayName() ); ?>">
                    <?php if ($isMine || COwnerHelper::isCommunityAdmin()) { ?>
                    <svg class="joms-icon" viewBox="0 0 16 16" onclick="joms.api.avatarChange('profile', '<?php echo $profile->id; ?>', arguments && arguments[0]); return false; ">
                        <use xlink:href="#joms-icon-camera">
                    </svg>
                    <?php } ?>
                </a>		
				<h2><strong><?php echo $user->getDisplayName(); ?></strong></h2>
...
?>
        ...
        <?php if ($loggedIn) { ?>
        <div class="hidden-xs joms-focus__button--options--desktop">
            <a id="options" href="javascript:" class="hidden-xs" data-ui-object="joms-dropdown-button">
                <svg viewBox="0 0 16 16" class="joms-icon">
