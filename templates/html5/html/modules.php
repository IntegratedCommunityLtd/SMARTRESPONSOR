 ...
 */
function modChrome_smart($module, &$params, &$attribs){ ?>
    <?php
	$badge = preg_match ('/badge/', $params->get('moduleclass_sfx')) ? "<span class=\"badges\"></span>\n" : "";
    ...
    $title = ($titles !== false) ? '<span class="first">'.JString::substr($module->title, 0, $titles).'</span> '.JString::substr($module->title, $titles+1) : $module->title; 
	?>

	<div class="module  <?php echo $modclass_sfx ?> clearfix">
	    
	    <?php if ((bool) $module->showtitle) : ?>
		    <h3 class="modtitle separatorline"> <?php echo $icons; ?><?php echo JText::_(''.$title.'') ?><?php echo $badge; ?></h3>
	    <?php endif; ?>
