<?php // Оригинал файла находится в models/mod_cf_filtering/tmpl/default.php
/**
	   ...
	   ?>
		...
		<div class="cf_flt_header" id="cfhead_<?php echo $key?>" role="tab">
			<div class="headexpand headexpand_<?php echo $state?>"	id="headexpand_<?php echo $key?>"></div>
				<label class="control-label cf_flt_header_text"><?php echo $filter_headers_array[$key]?></label>
		</div>
        ...
        ?>
		<div class="form-group cf_wrapper_inner" id="cf_wrapper_inner_<?php echo $key?>" role="tabpanel">
			<?php echo $flt_html?>
			<?php
			//$flt_html - здесь находятся все селекты для фильтра. Где разметка?
			?>
		</div>
	</div>
	<?php
