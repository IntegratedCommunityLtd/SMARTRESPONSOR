<?php
defined('_JEXEC') or die('Restricted access');if (!isset( $this->show )) $this->show = TRUE;
...
<?php   }
    ...
    <form id="com-form-login" action="<?php echo JRoute::_('index.php', $this->useXHTML, $this->useSSL); ?>" method="post" role="form" name="com-login" >
...
<?php  } else if ( $user->id ) { ?>
   ...
   <form action="<?php echo JRoute::_('index.php'); ?>" method="post" name="login" id="form-login">
        <span style="margin:4px;display:inline-block;"><?php echo JText::sprintf( 'COM_VIRTUEMART_HINAME', $user->name ); ?> </span>
