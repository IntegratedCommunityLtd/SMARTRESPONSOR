...
class VirtueMartViewVirtueMart extends VmView {
	...
	public function display($tpl = null) {
		...
		if(!VmConfig::get('shop_is_offline',0)){
			...
			vmJsApi::jPrice();
			if($vendorIdUser){ // Эту стр я раскоментировал (добавлять новый продукт)
				$user = JFactory::getUser(); // Эту стр  я раскоментировал (добавлять новый продукт)
				if( $vendorIdUser ){
			...
			} // Эту стр я раскоментировал (добавлять новый продукт) конец
			...
			if (!empty($featured_products_count) and VmConfig::get('show_featured', 1)) {
				...
				$productModel->addImages($this->products['featured'],10);
			}
			...
			if (!empty($latest_products_count) and VmConfig::get('show_latest', 1)) {
				...
				$productModel->addImages($this->products['latest'],10);
			}
			...
			if (!empty($topTen_products_count) and VmConfig::get('show_topTen', 1)) {
				...
				$productModel->addImages($this->products['topten'],10);
			}
			...
			if (!empty($recent_products_count) and VmConfig::get('show_recent', 1) and !empty($recent_products)) {
				...
				$productModel->addImages($this->products['recent'],10);
			}
